...
class DeltaCorrector:
    ...
    def fit(
        ...
        """
        logger.info(
            f"Fitting Delta corrector for variables: {variables} (method: {self.method})"
        )
        ...
        for var in variables:
            ...
            logger.info(
                f"Delta correction for {var}: bias={bias:.4f} "
                f"(model_{self.method}={model_center:.4f}, obs_{self.method}={obs_center:.4f})"
            )

        self.is_fitted = True
        logger.info(
            f"Delta corrector fitted for {len(self.bias_per_variable)} variables"
        )
