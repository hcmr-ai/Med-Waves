...
class EQMCorrector:
    ...
    def fit(
        ...
        for var in variables:
            ...
            if model_col not in df.columns or obs_col not in df.columns:
                logger.warning(
                    f"Columns {model_col} or {obs_col} not found, skipping {var}"
                )
                continue
    ...
    def _create_eqm_model(
        ...
        if use_kde:
            ...
            model_eval_points = np.linspace(
                model_min - 0.5 * model_range, model_max + 0.5 * model_range, 1000
            )
            obs_eval_points = np.linspace(
                obs_min - 0.5 * obs_range, obs_max + 0.5 * obs_range, 1000
            )
            ...
            model_quantiles = np.interp(
                quantile_levels, model_cdf_values, model_eval_points
            )
        ...
        logger.info(
            f"  Model: mean={model_stats['mean']:.4f}, std={model_stats['std']:.4f}"
        )
        logger.info(
            f"  Observed: mean={obs_stats['mean']:.4f}, std={obs_stats['std']:.4f}"
        )
    ...
    def predict(
        self, df: pl.DataFrame, variables: Optional[List[str]] = None
    ) -> pl.DataFrame:
        """
        ...
        for var in variables:
            ...
            logger.info(
                f"EQM correction for {var}: {n_corrected:,} values, bias reduction: {bias_reduction:.4f}"
            )
    ...
    def get_correction_stats(self) -> Dict[str, Dict]:
        ...
        for var, model in self.eqm_models.items():
            stats[var] = {
                ...
                "bias_relative": (
                    model["model_stats"]["mean"] - model["obs_stats"]["mean"]
                )
                / model["obs_stats"]["mean"]
                * 100,
