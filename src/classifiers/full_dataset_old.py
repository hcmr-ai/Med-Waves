        ...
        if max_samples is not None and sampling_strategy != "none":
            ...
            if original_size <= max_samples:
                ...
                if sampling_strategy == "random":
                    ...
                    samples_per_location = feature_config.get(
                    ...
                    df = stratified_sample_by_location(
                        df,
                        max_samples,
                    ...
                    df = temporal_sample_within_file(
                        df, samples_per_hour, sampling_seed
                    )
                else:
...
class FullDatasetTrainer:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        logger.info(
            f"FullDatasetTrainer initialized with model: {self.model_config.get('type', 'xgb')}"
        )
    ...
    def _initialize_model(self):
        ...
        if model_type == "xgb":
            self.model = xgb.XGBRegressor(
                ...
                early_stopping_rounds=self.model_config.get(
            ...
            logger.info(
                f"Initialized EQM corrector for variables: {self.eqm_variables}"
            )
            ...
            logger.info(
                f"Initialized Delta corrector for variables: {self.delta_variables} (method: {self.delta_method})"
            )
        else:
    ...
    def _initialize_preprocessing(self):
        ...
        if self.feature_config.get("feature_selection", {}).get("enabled", False):
            selection_type = self.feature_config["feature_selection"].get(
                "type", "kbest"
            )
        ...
        if self.feature_config.get("dimension_reduction", {}).get("enabled", False):
            reduction_type = self.feature_config["dimension_reduction"].get(
                "type", "pca"
            )
            n_components = self.feature_config["dimension_reduction"].get(
                "n_components", 50
            )
    ...
    def load_data(
        self, data_paths: Union[str, List[str]], target_column: str = "vhm0_y"
        ...
        if individual_files_count > 0:
            logger.info(
                f"Using {individual_files_count} individual file(s) directly (no S3 listing needed)"
            )
        if directory_paths_count > 0:
            logger.info(
                f"Expanding {directory_paths_count} directory path(s) to find files"
            )
        ...
        if max_samples is not None and sampling_strategy != "none":
            logger.info(
                f"Sampling configuration: {sampling_strategy} strategy, max {max_samples:,} samples per file"
            )
            if sampling_strategy == "per_location":
                samples_per_location = self.feature_config.get(
                    "samples_per_location", 20
                )
        ...
        if max_workers > 1:
            ...
            try:
                ...
                batch_size = (
                    max_workers * 2
                )  # Process 2x the number of workers at a time
                logger.info(f"Processing files in batches of {batch_size}")

                for i in tqdm(
                    range(0, len(all_files), batch_size), desc="Processing batches"
                ):
                    batch_files = all_files[i : i + batch_size]
                    batch_args = [
                        (file_path, self.feature_config) for file_path in batch_files
                    ]

                    with ProcessPoolExecutor(max_workers=max_workers) as executor:
                        # Submit batch tasks
                        futures = [
                            executor.submit(_load_single_file_worker, args)
                        ...
                        for future in futures:
                            try:
                                file_path_result, df, success = future.result(
                                    timeout=300
                                )  # 5 minute timeout
                                ...
                                if success and df is not None and len(df) > 0:
                                    ...
                                    if len(successful_files) <= 3:
                                        logger.info(
                                            f"File {len(successful_files)}: {len(df):,} samples loaded successfully"
                                        )
        ...
        if max_workers == 1:
            ...
            for file_path in tqdm(all_files, desc="Loading files"):
                try:
                    ...
                    if use_lag_features:
                        logger.debug(
                            f"Creating lag features for {file_path} before sampling..."
                        )
                        lag_config = self.feature_config.get("lag_features", {}).get(
                            "lags", {}
                        )
                    ...
                    if len(df) == 0:
                        logger.warning(
                            f"⚠️  File {file_path} has 0 samples after processing! Original: {original_size:,}"
                        )
                    else:
                        logger.debug(
                            f"File {file_path}: {original_size:,} → {len(df):,} samples"
                        )
                    ...
                    if len(df) > 0:
                        ...
                        if len(successful_files) <= 3:
                            ...
                            if original_size != final_size:
                                ...
                                logger.info(
                                    f"File {len(successful_files)}: {original_size:,} → {final_size:,} samples ({reduction:.1f}% reduction)"
                                )
                            else:
                                logger.info(
                                    f"File {len(successful_files)}: {original_size:,} samples (no sampling)"
                                )
        ...
        if max_samples is not None and sampling_strategy != "none":
            ...
            logger.info(
                f"Sampling summary: {total_samples:,} total samples from {len(successful_files)} files"
            )
        ...
        dataset_info = self._prepare_dataset_info(
            X, y, successful_files, max_samples, sampling_strategy
        )
    ...
    def _expand_s3_path(self, s3_uri: str) -> List[str]:
        ...
        parquet_files = list_s3_parquet_files(
            bucket, prefix, aws_profile, eval_months, train_end_year, test_start_year
        )
    ...
    def _apply_sampling(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        logger.info(
            f"Applying {sampling_strategy} sampling: max {max_samples:,} samples from {original_size:,} original samples"
        )

        if sampling_strategy == "per_location":
            samples_per_location = self.feature_config.get("samples_per_location", 20)
            logger.info(
                f"  - Per-location sampling: {samples_per_location} samples per location"
            )
            ...
            logger.info(
                f"  - Random sampling: selecting {max_samples:,} random samples"
            )
            sampled_df = random_sample_within_file(
                df, max_samples_per_file=max_samples, seed=sampling_seed
            )
        else:
        ...
        logger.info(
            f"Sampling completed: {original_size:,} → {final_size:,} samples ({reduction_percent:.1f}% reduction)"
        )

        return sampled_df

    def _prepare_dataset_info(
        self,
        X: np.ndarray,
        y: np.ndarray,
        ...
        if len(y) == 0:
            logger.error("❌ Empty dataset! No samples found after processing.")
            logger.error(f"   - Files processed: {len(successful_files)}")
            logger.error(f"   - Sampling strategy: {sampling_strategy}")
            logger.error(f"   - Max samples per file: {max_samples}")
        ...
        if max_samples is not None and sampling_strategy != "none":
            ...
            reduction_percent = (
                (estimated_original - len(X)) / estimated_original
            ) * 100
    ...
    def _prepare_features(
        ...
        # Handle NaN values intelligently, especially for lag features
        logger.info(f"Handling NaN values from {df.shape[0]} rows...")

        # Check if we have lag features
        lag_features = [col for col in df.columns if "_lag_" in col]

        if lag_features:
            logger.info(f"Found {len(lag_features)} lag features: {lag_features}")

            # For lag features, we expect some NaN values (first few rows of each location)
            # Only remove rows where ALL lag features are NaN (indicating insufficient temporal data)
            non_lag_features = [col for col in df.columns if "_lag_" not in col]

            # Count rows with different NaN patterns before filtering
            total_rows = df.shape[0]
            rows_all_lag_nan = df.filter(
                pl.all_horizontal([pl.col(col).is_null() for col in lag_features])
            ).shape[0]
            rows_some_lag_nan = df.filter(
                pl.any_horizontal([pl.col(col).is_null() for col in lag_features])
                & pl.any_horizontal([pl.col(col).is_not_null() for col in lag_features])
            ).shape[0]
            rows_no_lag_nan = df.filter(
                pl.all_horizontal([pl.col(col).is_not_null() for col in lag_features])
            ).shape[0]

            logger.info("Lag feature NaN analysis:")
            logger.info(f"  Total rows: {total_rows:,}")
            logger.info(f"  Rows with ALL lag features NaN: {rows_all_lag_nan:,}")
            logger.info(f"  Rows with SOME lag features NaN: {rows_some_lag_nan:,}")
            logger.info(f"  Rows with NO lag features NaN: {rows_no_lag_nan:,}")

            # More conservative approach: only remove rows where ALL lag features are null AND we have very few valid lag features
            # This preserves more data while still handling extreme cases
            df = df.filter(
                pl.any_horizontal([pl.col(col).is_not_null() for col in lag_features])
            )
            logger.info(
                f"After removing rows with all NaN lag features: {df.shape[0]} rows remaining"
            )

            # If we still have very few rows, try a more lenient approach
            if df.shape[0] < 1000:  # Arbitrary threshold
                logger.warning(
                    f"Very few rows remaining ({df.shape[0]}), trying more lenient NaN handling..."
                )
                # Revert to original data and try a different approach
                # This is a fallback - we'll implement this if needed

            # For remaining NaN values in lag features, fill with forward fill within each location
            for lag_col in lag_features:
                # Count NaN values before filling
                nan_count_before = df.select(pl.col(lag_col).is_null().sum()).item()
                logger.debug(
                    f"  {lag_col}: {nan_count_before:,} NaN values before forward fill"
                )
                ...
                # Count NaN values after forward fill
                nan_count_after_forward = df.select(
                    pl.col(lag_col).is_null().sum()
                ).item()
                logger.debug(
                    f"  {lag_col}: {nan_count_after_forward:,} NaN values after forward fill"
                )

                # If still NaN values, fill with backward fill
                if nan_count_after_forward > 0:
                    df = df.with_columns(
                        [
                            pl.col(lag_col)
                    ...
                    nan_count_after_backward = df.select(
                        pl.col(lag_col).is_null().sum()
                    ).item()
                    logger.debug(
                        f"  {lag_col}: {nan_count_after_backward:,} NaN values after backward fill"
                    )

                    # If still NaN values, fill with 0 (last resort)
                    if nan_count_after_backward > 0:
                        df = df.with_columns(
                            [pl.col(lag_col).fill_null(0).alias(lag_col)]
                        )
                        logger.warning(
                            f"  {lag_col}: Filled remaining {nan_count_after_backward:,} NaN values with 0"
                        )

            logger.info("Forward/backward filled remaining NaN values in lag features")

            # Now remove any remaining NaN values in non-lag features
            if non_lag_features:
                df = df.drop_nulls(subset=non_lag_features)
                logger.info(
                    f"After removing NaN values in non-lag features: {df.shape[0]} rows remaining"
                )
        else:
            # No lag features, use standard NaN removal
            df = df.drop_nulls()
            logger.info(f"After removing NaN values: {df.shape[0]} rows remaining")

        # Final safety check
        if df.shape[0] == 0:
            logger.error("❌ CRITICAL: No data remaining after NaN handling!")
            logger.error(
                "   This suggests the lag features are creating too many NaN values."
            )
            logger.error(
                "   Consider reducing lag values or using a different sampling strategy."
            )
            raise ValueError(
                "No data remaining after NaN handling. Check lag feature configuration."
            )
        ...
        # They are automatically included in feature_cols since they exist in the dataframe
        use_lag_features = self.feature_config.get("lag_features", {}).get(
            "enabled", False
        )
        if use_lag_features:
            ...
            # Count existing lag features in the dataframe
            lag_features_found = [col for col in df.columns if "_lag_" in col]
            logger.info(
    ...
    def _apply_regional_weights(
        self, X: np.ndarray, y: np.ndarray, regions: np.ndarray
    ) -> np.ndarray:
        """
        ...
        weights_config = self.feature_config.get("regional_weighting", {}).get(
            "weights", {}
        )
        ...
        for region, weight in weights_config.items():
            ...
            if region_count > 0:
                ...
                logger.info(
                    f"Applied weight {weight} to {region_count:,} {region} samples"
                )

        # Log weight statistics
        total_weighted_samples = np.sum(sample_weights)
        logger.info(
            f"Regional weighting applied - Total weighted samples: {total_weighted_samples:,.0f}"
        )
        ...
        for region_id in weights_config.keys():
            ...
            if np.sum(region_mask) > 0:
                ...
                region_name = RegionMapper.get_display_name(region_id)
                logger.info(
                    f"  {region_name}: {region_count:,} samples × {weights_config[region_id]} = {effective_count:,.0f} effective samples"
                )

        return sample_weights

    def _calculate_regional_metrics(
        self, y_true: np.ndarray, y_pred: np.ndarray, regions: np.ndarray
        ...
        for region_id in np.unique(regions):
            ...
            if np.sum(mask) > 0:
                ...
                region_name = RegionMapper.get_display_name(region_id)
                logger.info(
                    f"{region_name} metrics - RMSE: {region_metrics['rmse']:.4f}, "
    ...
    def _calculate_sea_bin_metrics(
        ...
        return calculate_sea_bin_metrics(
            y_true, y_pred, sea_bin_config, enable_logging=True
        )
    ...
    def split_data(
        ...
        logger.info(
            f"Data splits - Train: {self.X_train.shape}, Val: {self.X_val.shape}, Test: {self.X_test.shape}"
        )
    ...
    def _split_by_years(
                ...
                if year <= train_end_year:
                    ...
                    if year < train_end_year:
                        ...
                        if month in val_months:
                            ...
                            logger.info(
                                f"Validation file: {file_path} -> {year}-{month:02d}"
                            )
                        else:
                            train_files.append((file_path, start_idx, end_idx))
                            logger.debug(
                                f"Train file: {file_path} -> {year}-{month:02d}"
                            )
        ...
        logger.info(
            f"Regional distribution - Train: {dict(zip(train_names, train_counts, strict=False))}"
        )
        logger.info(f"Regional distribution - Val: {dict(zip(val_names, val_counts, strict=False))}")
        logger.info(
            f"Regional distribution - Test: {dict(zip(test_names, test_counts, strict=False))}"
        )
        ...
        for file_path, (_, _) in file_indices.items():
            try:
                date = extract_date_from_filename(file_path)
                year = date.year
                month = date.month
                ...
                if year < train_end_year:  # Training years
                    ...
                    year == train_end_year and month in val_months
                ):  # Validation months
        ...
        logger.info("Year-based split completed:")
        logger.info(f"  Train: {len(self.X_train)} samples, years: {train_years}")
        logger.info(
            f"  Val: {len(self.X_val)} samples, months {val_months} of {val_years}"
        )
        logger.info(
            f"  Test: {len(self.X_test)} samples, months {eval_months} of {test_years}"
        )
        ...
        split_info = {
            ...
            "train_ratio": len(self.X_train)
            / (len(self.X_train) + len(self.X_val) + len(self.X_test))
            if (len(self.X_train) + len(self.X_val) + len(self.X_test)) > 0
            else 0,
            "val_ratio": len(self.X_val)
            / (len(self.X_train) + len(self.X_val) + len(self.X_test))
            if (len(self.X_train) + len(self.X_val) + len(self.X_test)) > 0
            else 0,
            "test_ratio": len(self.X_test)
            / (len(self.X_train) + len(self.X_val) + len(self.X_test))
            if (len(self.X_train) + len(self.X_val) + len(self.X_test)) > 0
            else 0,
        }
    ...
    def preprocess_data(self) -> None:
        ...
        if self.regional_scaler is not None and self.regions_train is not None:
            ...
            self.X_train = self.regional_scaler.transform_with_regions(
                self.X_train, self.regions_train
            )

            if len(self.X_val) > 0:
                self.X_val = self.regional_scaler.transform_with_regions(
                    self.X_val, self.regions_val
                )
            else:
                logger.warning("Skipping validation set preprocessing (empty set)")

            if len(self.X_test) > 0:
                self.X_test = self.regional_scaler.transform_with_regions(
                    self.X_test, self.regions_test
                )
            else:
    ...
    def train(self) -> Dict[str, Any]:
        ...
        # Validate that we have data to train on
        if len(self.X_train) == 0:
            error_msg = "❌ Cannot train model: Empty training dataset!"
            logger.error(error_msg)
            raise ValueError(error_msg)

        logger.info(
            f"Training dataset: {len(self.X_train):,} samples, {self.X_train.shape[1]} features"
        )
        ...
        if isinstance(self.model, EQMCorrector):
            ...
            for var, stats in eqm_stats.items():
                logger.info(
                    f"EQM {var}: bias={stats['bias']:.4f}, relative_bias={stats['bias_relative']:.1f}%"
                )
        ...
        # Handle Delta corrector training
            ...
            self.model.fit(
                train_df, self.delta_variables, corrected_suffix="corrected_"
            )
            ...
            for var, stats in delta_stats.items():
                logger.info(
                    f"Delta {var}: bias={stats['bias']:.4f} (method: {stats['method']})"
                )
        ...
        if hasattr(self.model, "early_stopping_rounds"):
            ...
            if len(self.X_val) > 0:
                ...
                self.training_history["train_loss"] = self.model.evals_result_[
                    "validation_0"
                ]["rmse"]
                self.training_history["val_loss"] = self.model.evals_result_[
                    "validation_1"
                ]["rmse"]
            else:
                # No validation set, train without early stopping
                logger.warning(
                    "No validation set available, training without early stopping"
                )
                self.model.fit(
                    self.X_train,
                    self.y_train,
            ...
            try:
                ...
                logger.warning(
                    "Model doesn't support sample_weight, training without regional weighting"
                )
        ...
        sea_bin_train_metrics = self._calculate_sea_bin_metrics(
        ...
        logger.info(
            f"Training completed - Train RMSE: {self.train_metrics.get('rmse', 0):.4f}, Val RMSE: {self.val_metrics.get('rmse', 0):.4f}"
        )
        logger.info(
            f"Training SNR - Train: {self.train_metrics.get('snr', 0):.1f} ({self.train_metrics.get('snr_db', 0):.1f} dB), Val: {self.val_metrics.get('snr', 0):.1f} ({self.val_metrics.get('snr_db', 0):.1f} dB)"
        )
    ...
    def evaluate(self) -> Dict[str, Any]:
        ...
        logger.info(
            f"Test evaluation - RMSE: {self.test_metrics.get('rmse', 0):.4f}, MAE: {self.test_metrics.get('mae', 0):.4f}, Pearson: {self.test_metrics.get('pearson', 0):.4f}"
        )
        logger.info(
            f"Test SNR: {self.test_metrics.get('snr', 0):.1f} ({self.test_metrics.get('snr_db', 0):.1f} dB)"
        )
    ...
    def save_results_to_s3(
        self, results: Dict[str, Any], model_path: str = None, plots_dir: str = None
    ) -> Dict[str, bool]:
        """
        ...
        if not self.s3_results_saver.enabled:
            logger.warning(
                "S3 saving is disabled. Enable it in config to save results to S3."
            )
            return {}
        ...
        upload_results["results_json"] = self.s3_results_saver.save_results_json(
            results
        )
        ...
        if total_uploads == total_attempts:
            logger.info("✅ All results successfully uploaded to S3")
        else:
