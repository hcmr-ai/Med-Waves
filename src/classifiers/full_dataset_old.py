...
class FullDatasetTrainer:
        ...
        if max_workers == 1:
            ...
            for file_path in tqdm(all_files, desc="Loading files"):
                try:
                    ...
                    if len(df) == 0:
                        logger.warning(f"⚠️  File {file_path} has 0 samples after processing! Original: {original_size:,}")
                    else:
                        logger.debug(f"File {file_path}: {original_size:,} → {len(df):,} samples")
    ...
    def _prepare_features(self, df: pl.DataFrame, target_column: str) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        ...
        if lag_features:
            ...
            logger.info(f"Lag feature NaN analysis:")
            ...
            logger.info(f"After removing rows with all NaN lag features: {df.shape[0]} rows remaining")
            
            # If we still have very few rows, try a more lenient approach
            if df.shape[0] < 1000:  # Arbitrary threshold
                logger.warning(f"Very few rows remaining ({df.shape[0]}), trying more lenient NaN handling...")
            ...
            for lag_col in lag_features:
                ...
                logger.debug(f"  {lag_col}: {nan_count_before:,} NaN values before forward fill")
                
                # Try forward fill first
                df = df.with_columns([
                    pl.col(lag_col).fill_null(strategy="forward").over(["lat", "lon"]).alias(lag_col)
                ])
                
                # Count NaN values after forward fill
                nan_count_after_forward = df.select(pl.col(lag_col).is_null().sum()).item()
                logger.debug(f"  {lag_col}: {nan_count_after_forward:,} NaN values after forward fill")
                
                # If still NaN values, fill with backward fill
                if nan_count_after_forward > 0:
                    df = df.with_columns([
                        pl.col(lag_col).fill_null(strategy="backward").over(["lat", "lon"]).alias(lag_col)
                    ])
                    nan_count_after_backward = df.select(pl.col(lag_col).is_null().sum()).item()
                    logger.debug(f"  {lag_col}: {nan_count_after_backward:,} NaN values after backward fill")
                    ...
                    if nan_count_after_backward > 0:
                        ...
                        logger.warning(f"  {lag_col}: Filled remaining {nan_count_after_backward:,} NaN values with 0")
            ...
            if non_lag_features:
                ...
                logger.info(f"After removing NaN values in non-lag features: {df.shape[0]} rows remaining")
        else:
        ...
        if df.shape[0] == 0:
            ...
            logger.error("   This suggests the lag features are creating too many NaN values.")
            logger.error("   Consider reducing lag values or using a different sampling strategy.")
            raise ValueError("No data remaining after NaN handling. Check lag feature configuration.")
    ...
    def train(self) -> Dict[str, Any]:
        ...
        logger.info(f"Training dataset: {len(self.X_train):,} samples, {self.X_train.shape[1]} features")
