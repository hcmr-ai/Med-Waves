...
def train_single_cluster_dask(
    ...
    # X_train_cluster = scattered_data['X_train']
    # X_val_cluster = scattered_data['X_val']
    # X_test_cluster = scattered_data['X_test']
    # y_train_cluster = scattered_data['y_train']
    # y_val_cluster = scattered_data['y_val']
    # y_test_cluster = scattered_data['y_test']
    ...
    # regions_train_cluster = scattered_data['regions_train']
    # regions_test_cluster = scattered_data['regions_test']
    # vhm0_x_train_cluster = scattered_data['vhm0_x_train']
    # vhm0_x_test_cluster = scattered_data['vhm0_x_test']
    # sample_weights_cluster = scattered_data['sample_weights']
    ...
    model = xgb.XGBRegressor(
        ...
        objective=model_config.get("objective", "reg:pseudohubererror"),
        huber_slope=model_config.get("huber_slope", 0.5),
...
class ModelPerPointTrainer:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.sampling_manager = (
            SamplingManager(config)
            if config.get("feature_block", {}).get("sampling_strategy", None)
            else None
        )
        ...
        logger.info(
            f"Model Per cluster initialized with model: {self.model_config.get('type', 'xgb')}"
        )
    ...
    def load_data(
        ...
        X, y, regions, coords, self.unique_clusters, cluster_ids = (
            self.feature_engineer.prepare_features(combined_df, target_column)
        )

        # Get metadata
        years, months, self.vhm0_x_raw, actual_wave_heights = (
            self.feature_engineer.prepare_metadata(combined_df, target_column)
        )
    ...
    def _prepare_dataset_info(
        self, X: np.ndarray, y: np.ndarray, successful_files: List[str]
    ) -> Dict[str, Any]:
        """Prepare dataset information for logging."""
        dataset_info = {
            "total_samples": X.height,
            "total_features": X.width,
        ...
        if self.sampling_manager:
            sampling_reduction = self.sampling_manager.estimate_sampling_reduction(
                len(successful_files), len(X)
            )
    ...
    def _calculate_regional_metrics(
        ...
        return self.metrics_calculator.calculate_regional_metrics(
    ...
    def _calculate_sea_bin_metrics(
        ...
        return self.metrics_calculator.calculate_sea_bin_metrics(
            y_true, y_pred, enable_logging=True
        )

    def _calculate_region_sea_bin_metrics(
        self,
        y_true: np.ndarray,
        y_pred: np.ndarray,
        regions: np.ndarray,
        ...
        """Calculate metrics for different sea state bins using MetricsCalculator."""
        return self.metrics_calculator.calculate_region_sea_bin_metrics(
            y_true, y_pred, regions, vhm0_x_test
        )
    ...
    def split_data(
        ...
        vhm0_x: np.ndarray = None,
        actual_wave_heights: np.ndarray = None,
        ...
        self.regions_train = split_data["regions_train"].to_numpy()
        self.regions_val = split_data["regions_val"].to_numpy()
        self.regions_test = split_data["regions_test"].to_numpy()
        self.coords_train = split_data["coords_train"].to_numpy()
        self.coords_val = split_data["coords_val"].to_numpy()
        self.coords_test = split_data["coords_test"].to_numpy()
        ...
        if vhm0_x is not None:
            ...
            if train_indices is not None:
                ...
                self.vhm0_x_val = (
                    vhm0_x[val_indices] if val_indices is not None else pl.DataFrame()
                )
                self.vhm0_x_test = (
                    vhm0_x[test_indices] if test_indices is not None else pl.DataFrame()
                )
            else:
                # Fallback: assume equal distribution
                logger.warning(
                    "DataSplitter didn't return indices. Using fallback splitting for vhm0_x."
                )
                n_samples = vhm0_x.height
                n_train = self.X_train.height
                n_val = self.X_val.height

                self.vhm0_x_train = vhm0_x.slice(0, n_train)
                self.vhm0_x_val = (
                    vhm0_x.slice(n_train, n_val) if n_val > 0 else pl.DataFrame()
                )
                self.vhm0_x_test = (
                    vhm0_x.slice(n_train + n_val, n_samples - (n_train + n_val))
        ...
        if self.predict_bias and actual_wave_heights is not None:
            ...
            actual_wave_heights_train = split_data.get(
                "actual_wave_heights_train", None
            )
            if actual_wave_heights_train is not None:
                self.sample_weights = self.sample_weighting.apply_weights(
                    actual_wave_heights_train.to_numpy(), self.regions_train
                )
                logger.info(
                    "Applied sample weights based on actual wave heights (bias prediction mode)"
                )
            else:
                # Fallback to y_train if actual_wave_heights_train is not available
                self.sample_weights = self.sample_weighting.apply_weights(
                    self.y_train.to_numpy(), self.regions_train
                )
                logger.warning(
                    "actual_wave_heights_train not found in split_data, using y_train for weighting"
                )
        else:
        ...
        del actual_wave_heights_train, actual_wave_heights
        import gc
    ...
    def preprocess_data(self) -> None:
        ...
        self.X_train, self.X_val, self.X_test = (
            self.preprocessing_manager.preprocess_splits_memory_efficient(
    ...
    def _train_per_point(self, cluster_id: int) -> None:
        ...
        # Convert cluster selection to polars filtering
        mask_train = self.cluster_ids_train == cluster_id
        mask_val = (
            self.cluster_ids_val == cluster_id
            if self.cluster_ids_val is not None
            else None
        )

        X_train_cluster = self.X_train[mask_train]
        y_train_cluster = self.y_train.to_numpy()[mask_train].ravel()

        X_val_cluster = (
            self.X_val[mask_val]
            if mask_val is not None and len(self.X_val) > 0
            else np.empty((0, self.X_train.shape[1]))
        )
        y_val_cluster = (
            self.y_val[mask_val].ravel()
            if mask_val is not None and len(self.y_val) > 0
            else np.empty((0,))
        )

        vhm0_x_train_cluster = (
            self.vhm0_x_train.to_numpy()[mask_train]
            if self.vhm0_x_train is not None
            else None
        )
        sample_weights_cluster = (
            self.sample_weights[mask_train] if self.sample_weights is not None else None
        )
        regions_train_cluster = self.regions_train[mask_train]
        ...
        if len(X_val_cluster) > 0:
            ...
            del X_val_cluster, y_val_cluster, mask_val
        else:
        ...
        del X_train_cluster, y_train_cluster, train_pred, mask_train
        del sample_weights_cluster
        import gc
        ...
        return (
            ...
            vhm0_pred_train,
            vhm0_x_train_cluster,
            regions_train_cluster,
        )

    def _train_and_eval_cluster(self, cluster_id: int):
        """Helper: train + evaluate on a single cluster, return metrics + preds."""
        # Train on cluster
        vhm0_y_train, vhm0_pred_train, vhm0_x_train_cluster, regions_train_cluster = (
            self._train_per_point(cluster_id)
        )
        # Evaluate on test set
        vhm0_y_test, vhm0_pred_test, vhm0_x_test_cluster, regions_test_cluster = (
            self.evaluate(cluster_id)
        )
        return {
            "cluster_id": cluster_id,
            "train_true": vhm0_y_train,
    ...
    def train(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # cluster_data = {
            #     cid: {
            #         "X_train": X_train_np[self.cluster_ids_train == cid],
            #         "y_train": self.y_train.to_numpy()[self.cluster_ids_train == cid].ravel(),
            #         "X_val": self.X_val[self.cluster_ids_val == cid] if self.X_val is not None else None,
            #         "y_val": self.y_val.to_numpy()[self.cluster_ids_val == cid].ravel() if self.y_val is not None else None,
            #         "X_test": X_test_np[self.cluster_ids_test == cid],
            #         "y_test": self.y_test.to_numpy()[self.cluster_ids_test == cid].ravel(),
            #         "regions_train": self.regions_train[self.cluster_ids_train == cid],
            #         "regions_test": self.regions_test[self.cluster_ids_test == cid],
            #         "vhm0_x_train": self.vhm0_x_train.to_numpy()[self.cluster_ids_train == cid] if self.vhm0_x_train is not None else None,
            #         "vhm0_x_test": self.vhm0_x_test.to_numpy()[self.cluster_ids_test == cid] if self.vhm0_x_test is not None else None,
            #         "sample_weights": self.sample_weights[self.cluster_ids_train == cid] if self.sample_weights is not None else None,
            #     }
            #     for cid in self.unique_clusters
            # }
            # scattered_data = client.scatter(cluster_data, broadcast=False)
        ...
        vhm0_x_train_cluster = (
            np.concatenate(vhm0_x_train) if len(vhm0_x_train) > 0 else None
        )
        regions_train_cluster = np.concatenate(regions_train)
        self.vhm0_y_train = vhm0_y_train

        vhm0_y_test = (
            np.concatenate(y_trues_test) if len(y_trues_test) > 0 else np.array([])
        )
        vhm0_pred_test = (
            np.concatenate(y_preds_test) if len(y_preds_test) > 0 else np.array([])
        )
        vhm0_x_test_cluster = (
            np.concatenate(vhm0_x_test) if len(vhm0_x_test) > 0 else np.array([])
        )
        regions_test_cluster = (
            np.concatenate(regions_test) if len(regions_test) > 0 else np.array([])
        )
        self.vhm0_y_test = vhm0_y_test

        del y_trues_test, y_preds_test, regions_test, vhm0_x_test
        import gc

        gc.collect()

        train_metrics = self._train_metrics_and_plots(
            vhm0_y_train, vhm0_pred_train, vhm0_x_train_cluster, regions_train_cluster
        )
        ...
        if len(vhm0_y_test) == 0:
            logger.warning(
                "Test set is empty after training! Skipping evaluation metrics."
            )
    ...
    def _train_metrics_and_plots(
        self,
        y_train: np.ndarray,
        y_pred: np.ndarray,
        vhm0_x_train: np.ndarray,
        ...
        if self.vhm0_x_train is not None:
            ...
            baseline_regional_train_metrics = self._calculate_regional_metrics(
                y_train, vhm0_x_train, regions_train
            )
            logger.info("Calculating baseline sea-bin training metrics...")
            baseline_sea_bin_train_metrics = self._calculate_sea_bin_metrics(
                y_train, vhm0_x_train
            )
        else:
            logger.warning(
                "vhm0_x_train not available. Skipping baseline regional training metrics."
            )
        ...
        self.experiment_logger.log_training_results(
    ...
    def evaluate(
        ...
        mask_test = self.cluster_ids_test == cluster_id
        X_test_cluster = self.X_test[mask_test]
        y_test_cluster = self.y_test.to_numpy()[mask_test].ravel()
        vhm0_x_test_cluster = (
            self.vhm0_x_test.to_numpy()[mask_test]
            if self.vhm0_x_test is not None
            else None
        )
        regions_test_cluster = self.regions_test[mask_test]
    ...
    def _evaluation_metrics_and_plots(
        self,
        y_true: np.ndarray,
        y_pred: np.ndarray,
        vhm0_x_test: np.ndarray,
        ...
        if len(self.X_test) > 0:
            ...
            if vhm0_x_test is not None and len(vhm0_x_test) > 0:
                ...
                logger.warning(
                    "vhm0_x_test not available. Skipping baseline regional test metrics."
                )
        ...
        self.region_sea_bin_metrics = self._calculate_region_sea_bin_metrics(
            y_true, y_pred, regions_test, vhm0_x_test
        )
