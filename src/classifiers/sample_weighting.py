...
class SampleWeighting:
    ...
    def _apply_regional_weights(self, y: np.ndarray, regions: np.ndarray) -> np.ndarray:
        ...
        if regions is None:
            logger.warning(
                "Regional weighting enabled but no region information provided"
            )
        ...
        for region, weight in weights_config.items():
            ...
            if region_count > 0:
                ...
                logger.info(
                    f"Applied regional weight {weight} to {region_count:,} {region} samples"
                )
    ...
    def _log_wave_height_bin_statistics(
        self, y: np.ndarray, wave_weights: np.ndarray
    ) -> None:
        ...
        for min_hs, max_hs, label in bins:
            ...
            if count > 0:
                ...
                logger.info(
                    f"  {label}: {count:,} samples, avg weight: {avg_weight:.2f}, effective: {effective_count:,.0f}"
                )

    def _log_weight_statistics(
        self, sample_weights: np.ndarray, y: np.ndarray, regions: np.ndarray = None
    ) -> None:
        ...
        logger.info(
            f"Weight range: {np.min(sample_weights):.3f} - {np.max(sample_weights):.3f}"
        )
        ...
        if regions is not None and self.regional_config.get("enabled", False):
            ...
            for region_id in weights_config.keys():
                ...
                if np.sum(region_mask) > 0:
                    ...
                    logger.info(
                        f"  {region_name}: {region_count:,} samples → {effective_count:,.0f} effective samples"
                    )
