...
class FullDatasetTrainer:
    ...
    def split_data(
        ...
        split_data = self.data_splitter.split_data(
            X, y, regions, coords, file_paths, actual_wave_heights, years, months
        )
        ...
        if vhm0_x is not None:
            ...
            if train_indices is not None:
                ...
                self.vhm0_x_val = (
                    vhm0_x[val_indices] if val_indices is not None else np.array([])
                )
                self.vhm0_x_test = (
                    vhm0_x[test_indices] if test_indices is not None else np.array([])
                )
            else:
                ...
                self.vhm0_x_val = (
                    vhm0_x[n_train : n_train + n_val] if n_val > 0 else np.array([])
                )
                self.vhm0_x_test = (
                    vhm0_x[n_train + n_val :]
    ...
    def train(self) -> Dict[str, Any]:
        ...
        if len(self.X_val) > 0:
            ...
            if self.vhm0_x_val is not None and len(self.vhm0_x_val) > 0:
                baseline_regional_val_metrics = self._calculate_regional_metrics(
                    vhm0_y_val, self.vhm0_x_val, self.regions_val
                )
            else:
                logger.warning(
                    "vhm0_x_val not available. Skipping baseline regional validation metrics."
                )
                baseline_regional_val_metrics = {}

            sea_bin_val_metrics = self._calculate_sea_bin_metrics(
                vhm0_y_val, vhm0_pred_val
            )
        ...
        return {
            ...
            "baseline_regional_val_metrics": baseline_regional_val_metrics
            if len(self.X_val) > 0
            else {},
    ...
    def evaluate(self) -> Dict[str, Any]:
        ...
        if len(self.X_test) > 0:
            ...
            sea_bin_test_metrics = self._calculate_sea_bin_metrics(
                vhm0_y_test, vhm0_pred_test
            )
        else:
            test_metrics = {
                "rmse": 0.0,
        ...
        self.region_sea_bin_metrics = self._calculate_region_sea_bin_metrics(
            vhm0_y_test, vhm0_pred_test, self.regions_test, self.vhm0_x_test
        )
