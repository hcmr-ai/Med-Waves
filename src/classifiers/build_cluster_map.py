...
def plot_cluster_distribution_suite(df: pl.DataFrame, output_dir: str = "."):
    ...
    plt.tight_layout()
    plt.savefig(
        f"{output_dir}/cluster_distribution_suite.png", dpi=300, bbox_inches="tight"
    )
...
def plot_top_clusters(cluster_counts, ax, top_n=20):
    ...
    for i, (_, count) in enumerate(zip(bars, counts, strict=False)):
        ax.text(count + counts.max() * 0.01, i, f"{count:,}", va="center", fontsize=8)
...
def plot_spatial_heatmap(df: pl.DataFrame, output_dir: str = "."):
    ...
    cluster_stats = df.group_by("cluster_id").agg(
        [
    ...
    plt.tight_layout()
    plt.savefig(
        f"{output_dir}/spatial_cluster_heatmap.png", dpi=300, bbox_inches="tight"
    )
...
def plot_imbalance_analysis(df: pl.DataFrame, output_dir: str = "."):
    ...
    ax1.bar(
        range(len(deviation)), deviation, color=colors, alpha=0.7, edgecolor="black"
    )
    ...
    ax2.fill_between(
        range(len(lorenz)), lorenz, np.linspace(0, 1, len(lorenz)), alpha=0.3
    )
    ...
    cat_labels = [
        f"{label}\n({count} clusters)"
        for (label, _), count in zip(categories, cat_counts, strict=False)
    ]
    ...
    stats_data = [
        ...
        [
            "Clusters < Mean",
            f"{(counts < counts.mean()).sum()} ({(counts < counts.mean()).sum() / len(counts) * 100:.1f}%)",
        ],
        [
            "Clusters > 2×Mean",
            f"{(counts > 2 * counts.mean()).sum()} ({(counts > 2 * counts.mean()).sum() / len(counts) * 100:.1f}%)",
        ],
    ]
    ...
    plt.tight_layout()
    plt.savefig(
        f"{output_dir}/cluster_imbalance_analysis.png", dpi=300, bbox_inches="tight"
    )
...
class PointClusterMapper:
    ...
    def get_cluster_id_old(
        self, lat_series: pl.Series, lon_series: pl.Series
    ) -> pl.Series:
        """
