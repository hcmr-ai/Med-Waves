...
def run_experiment(config: Dict[str, Any], data_files: List[str], save_path: str) -> Dict[str, Any]:
    ...
    logger.info(f"Starting Model Per point training experiment with {len(data_files)} files")
    ...
    X, y, regions, coords, successful_files, actual_wave_heights, years, months, cluster_ids = trainer.load_data(data_files, config["data"]["target_column"])
    x_shape = X.shape
    y_shape = y.shape
