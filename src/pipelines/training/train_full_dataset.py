...
def run_experiment(config: Dict[str, Any], data_files: List[str], save_path: str) -> Dict[str, Any]:
    """Run the full dataset training experiment."""
    logger.info(f"Starting full dataset training experiment with {len(data_files)} files")
    ...
    X, y, regions, coords, successful_files, actual_wave_heights = trainer.load_data(data_files, config["data"]["target_column"])
    x_shape = X.shape
    y_shape = y.shape
    
    # Split data
    logger.info("Splitting data...")
    trainer.split_data(X, y, regions, coords, successful_files, trainer.vhm0_x_raw, actual_wave_heights)
    ...
    save_results(convert_numpy_types(results), save_path, trainer.experiment_logger.experiment)
    ...
    if s3_config.get("enabled", False):
        ...
        s3_upload_results = trainer.save_results_to_s3(convert_numpy_types(results), model_save_path, plots_dir)
...
def save_results(results: Dict[str, Any], save_path: str, comet_experiment: Experiment = None) -> None:
    ...
    with open(summary_file, 'w') as f:
        ...
        f.write(f"\nTraining Results:\n")
        f.write(f"  Train RMSE: {results['training_results']['train_metrics']['rmse']:.4f}\n")
        f.write(f"  Train MAE: {results['training_results']['train_metrics']['mae']:.4f}\n")
        f.write(f"  Train Pearson: {results['training_results']['train_metrics']['pearson']:.4f}\n")
        f.write(f"  Val RMSE: {results['training_results']['val_metrics']['rmse']:.4f}\n")
        f.write(f"  Val MAE: {results['training_results']['val_metrics']['mae']:.4f}\n")
        f.write(f"  Val Pearson: {results['training_results']['val_metrics']['pearson']:.4f}\n")
        f.write(f"\nTest Results:\n")
        f.write(f"  Test RMSE: {results['evaluation_results']['test_metrics']['rmse']:.4f}\n")
        f.write(f"  Test MAE: {results['evaluation_results']['test_metrics']['mae']:.4f}\n")
        f.write(f"  Test Pearson: {results['evaluation_results']['test_metrics']['pearson']:.4f}\n")
...
def main():
    ...
    print(f"  Train Pearson: {results['training_results']['train_metrics']['pearson']:.4f}")
