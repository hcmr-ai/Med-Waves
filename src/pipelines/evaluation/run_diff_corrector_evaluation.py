...
def main():
    ...
    class FilteredPredictionPlotter(PredictionPlotter):
        ...
        def _load_all_predictions(self) -> pl.DataFrame:
            ...
            print(
                f"📂 Loading daily predictions from {self.prediction_dir} with patterns: {self.patterns}"
            )
        ...
        def compute_metrics(self, resolution: float = 0.25):
            ...
            if not all_files:
                raise FileNotFoundError(
                    f"No files matching patterns {self.patterns} found in {self.prediction_dir}"
                )
            ...
            def metric_block(uncorrected: str, corrected: str):
                ...
                return (
                    ((pl.col(uncorrected) - pl.col(corrected)) ** 2)
                    .mean()
                    .sqrt()
                    .alias(f"rmse_{base}"),
                    (pl.col(uncorrected) - pl.col(corrected))
                    .abs()
                    .mean()
                    .alias(f"mae_{base}"),
                    (pl.col(uncorrected) - pl.col(corrected))
                    .mean()
                    .alias(f"diff_{base}"),
                    (pl.col(uncorrected).mean() - pl.col(corrected).mean()).alias(
                        f"bias_{base}"
                    ),
                    ((pl.col(uncorrected) - pl.col(uncorrected).mean()) ** 2)
                    .mean()
                    .alias(f"var_{base}"),
    ...
    try:
        ...
        metrics_day, metrics_month, metrics_spatial = plotter.compute_metrics(
            resolution=0.25
        )
        ...
        metrics_spatial.write_parquet(
            predictions_dir / "metrics_spatial_by_month.parquet"
        )
        ...
        print(
            f"   - metrics_spatial_by_month.parquet ({len(metrics_spatial)} spatial bins)"
        )
