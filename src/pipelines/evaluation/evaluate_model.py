...
class ModelEvaluator:
    ...
    def __init__(self, model_path: str, output_dir: str, config: Dict[str, Any] = None):
        ...
        if self.parallel_enabled:
            logger.info(
                f"Parallel processing: {self.n_workers} workers, batch size: {self.batch_size}"
            )
        else:
    ...
    def load_model(self) -> None:
        ...
        if (
            self.s3_model_loader
            and self.s3_model_loader.enabled
            and self.s3_model_loader._is_s3_path(str(self.model_path))
        ):
            ...
            self.feature_columns = components.get("selected_features")
        ...
        features_file = self.model_path / "selected_features.pkl"
        if features_file.exists():
            ...
            if self.feature_columns is None:
                ...
                logger.info(
                    f"Loaded feature columns: {len(self.feature_columns)} features"
                )
    ...
    def evaluate_files_parallel(self, data_files: List[str]) -> Dict[str, Any]:
        ...
        logger.info(
            f"⚙️ Using {self.n_workers} workers with batch size {self.batch_size}"
        )

        # Prepare arguments for workers
        worker_args = [
            (file_path, str(self.model_path), self.config) for file_path in data_files
        ]
                        ...
                        for future in as_completed(future_to_file):
                            ...
                            try:
                                ...
                                if result is not None:
                                    ...
                                    logger.info(
                                        f"✅ Evaluated {file_name}: RMSE={rmse:.4f}, MAE={mae:.4f}"
                                    )
        ...
        logger.info(
            f"Parallel evaluation completed: {successful_evaluations}/{len(data_files)} files successful"
        )
    ...
    def _get_data_files(
        self, data_path: str, year: str = None, file_pattern: str = None
    ) -> List[str]:
        """
        ...
        pattern = file_pattern or self.config.get("data", {}).get(
    ...
    def load_and_preprocess_file(
        self, file_path: str, return_raw: bool = False
        ...
        Returns:
            Tuple of (X, y, regions, coords, metadata_df, vhm0_x_index) or (None, None, None, None, None, None) if failed
        """
        ...
        try:
            ...
            # Get vhm0_x index directly from dataframe columns
            vhm0_x_index = None
            if "vhm0_x" in df.columns:
                vhm0_x_index = df.columns.index("vhm0_x")

            # Prepare features using the same logic as trainer
            X_raw, y_raw, regions_raw, coords_raw = (
                self._prepare_features_from_dataframe(df)
            )
            ...
            if return_raw:
                return X_raw, y_raw, regions_raw, coords_raw, metadata_df, vhm0_x_index
            else:
                return X, y_raw, regions_raw, coords_raw, metadata_df, vhm0_x_index
    ...
    def _prepare_features_from_dataframe(
        ...
        feature_cols = [
            ...
            if col
            not in ["vhm0_y"]
            + self.config.get("feature_block", {}).get("features_to_exclude", [])
        ...
        if use_geo_context:
            ...
            df = df.with_columns(
                [
                    ...
                    (
                        (pl.col("longitude") - (-5.5)) ** 2
                        + (pl.col("latitude") - 36.0) ** 2
                    )
                    .sqrt()
                    .alias("dist_from_gibraltar"),
                    ((pl.col("longitude") - 0) ** 2 + (pl.col("latitude") - 40) ** 2)
                    .sqrt()
            ...
            if "time" in df.columns or "timestamp" in df.columns:
                ...
                for variable, lags in lag_config.items():
                    if variable in df.columns:
                        logger.info(
                            f"Creating lag features for {variable} with lags: {lags}"
                        )
                        for lag in lags:
                        ...
                        logger.warning(
                            f"Variable {variable} not found in data for lag features"
                        )

                # Note: NaN values in lag features are handled by the initial drop_nulls() call

                logger.info(
                    f"Added {len(lag_features_added)} lag features: {lag_features_added}"
                )
                ...
                if "time" in feature_cols:
                    ...
                    logger.info(
                        "Removed 'time' column from features after lag feature creation"
                    )
            else:
                logger.warning(
                    "Lag features enabled but no time column found. Skipping lag features."
                )
        ...
        use_regional_training = (
            self.config.get("feature_block", {})
            .get("regional_training", {})
            .get("enabled", False)
        )
        ...
        logger.info(
            f"Final dataset - X: {X_raw.shape}, y: {y_raw.shape}, coords: {coords_raw.shape}"
        )
        if regions_raw is not None:
            unique_regions = np.unique(regions_raw)
            region_names = [
                RegionMapper.get_display_name(rid) for rid in unique_regions
            ]
            logger.info(
                f"Regional distribution: {region_names} (IDs: {unique_regions})"
            )

        return X_raw, y_raw, regions_raw, coords_raw

    def _calculate_sea_bin_metrics(
        ...
        return calculate_sea_bin_metrics(
            y_true, y_pred, sea_bin_config, enable_logging=False
        )

    def _apply_preprocessing(
        self, X: np.ndarray, regions: np.ndarray = None
    ) -> np.ndarray:
        """
    ...
    def evaluate_file(self, file_path: str) -> Dict[str, Any]:
        ...
        X, y, regions, coords, metadata_df, vhm0_x_index = (
            self.load_and_preprocess_file(file_path, return_raw=True)
        )
        ...
        # Compute baseline metrics (vhm0_x vs vhm0_y) - direct comparison without model
        if vhm0_x_index is not None:
            vhm0_x = X[:, vhm0_x_index]
            baseline_metrics = evaluate_model(vhm0_x, y)
            logger.info(
                f"Baseline metrics (vhm0_x vs vhm0_y): RMSE={baseline_metrics['rmse']:.4f}, MAE={baseline_metrics['mae']:.4f}, Bias={baseline_metrics['bias']:.4f}"
            )
        else:
            logger.warning(
                "vhm0_x column not found in features - skipping baseline metrics"
            )
        ...
        # Compute comprehensive spatial metrics for model predictions
        ...
        # Compute baseline spatial metrics (vhm0_x vs vhm0_y)
        baseline_spatial_metrics_df = None
        if vhm0_x_index is not None:
            baseline_spatial_df = metadata_df.copy()
            baseline_spatial_df["y_true"] = y
            baseline_spatial_df["y_pred"] = vhm0_x
            baseline_spatial_df["residual"] = vhm0_x - y

            # Convert to polars for spatial metrics computation
            baseline_spatial_pl = pl.from_pandas(baseline_spatial_df)
            baseline_spatial_metrics_df = self._compute_comprehensive_spatial_metrics(
                baseline_spatial_pl
            )
        ...
        results = {
            ...
            "baseline_spatial_metrics": baseline_spatial_metrics_df.to_pandas()
            if baseline_spatial_metrics_df is not None
            else None,
            "predictions": {"y_true": y, "y_pred": y_pred, "residuals": y_pred - y},
        ...
        logger.info(
            f"✅ Evaluated {file_name}: RMSE={metrics['rmse']:.4f}, MAE={metrics['mae']:.4f}, Bias={metrics['bias']:.4f}"
        )
        return results

    def create_spatial_maps(self, results: Dict[str, Any]) -> None:
        """
        Create spatial maps for a single file's results (both model and baseline).
        ...
        baseline_spatial_metrics = results.get("baseline_spatial_metrics")
        # metadata = results["metadata"]

        logger.info(f"Creating spatial maps for {file_name}")

        # Create model spatial maps
        self._create_single_spatial_maps(file_name, spatial_metrics, "model")

        # Create baseline spatial maps if available
        if baseline_spatial_metrics is not None:
            self._create_single_spatial_maps(
                file_name, baseline_spatial_metrics, "baseline"
            )

    def _create_single_spatial_maps(
        self, file_name: str, spatial_metrics: pd.DataFrame, map_type: str
    ) -> None:
        """
        Create spatial maps for a single type (model or baseline).

        Args:
            file_name: Name of the file
            spatial_metrics: Spatial metrics DataFrame
            map_type: Type of maps ("model" or "baseline")
        """
        logger.info(f"Creating {map_type} spatial maps for {file_name}")

        # Determine which coordinate columns to use (same logic as spatial metrics)
        if (
            "latitude" in spatial_metrics.columns
            and "longitude" in spatial_metrics.columns
        ):
        ...
        for metric in self.metrics_to_plot:
            ...
            save_path = (
                self.output_dir
                / "spatial_maps"
                / f"{file_name}_{map_type}_{metric}_map.png"
            )
            title = f"{metric.upper()} - {file_name} ({map_type})"
            ...
            try:
                plot_spatial_feature_map(
                    ...
                    s=self.config.get("evaluation", {})
                    .get("plots", {})
                    .get("marker_size", 8),
                    alpha=self.config.get("evaluation", {})
                    .get("plots", {})
                    .get("alpha", 0.85),
                    cmap=self.config.get("evaluation", {})
                    .get("plots", {})
    ...
    def create_spatial_maps_parallel(self) -> None:
        ...
        logger.info(
            f"🗺️ Creating spatial maps for {len(results_to_process)} files in parallel..."
        )
                ...
                for future in as_completed(future_to_file):
                    ...
                    try:
                        ...
                        if success:
                            ...
                            logger.warning(
                                f"❌ Failed to create spatial maps for {file_name}"
                            )
                    except Exception as e:
                        logger.error(
                            f"❌ Error creating spatial maps for {file_name}: {e}"
                        )
                    finally:
                        pbar.update(1)
                        pbar.set_postfix(
        ...
        logger.info(
            f"✅ Spatial map creation completed: {successful_maps}/{len(results_to_process)} files successful"
        )
    ...
    def create_aggregated_spatial_maps(self) -> None:
        """Create aggregated spatial maps across all files (both model and baseline)."""
        ...
        # Create model aggregated spatial maps
        self._create_single_aggregated_spatial_maps("model")

        # Create baseline aggregated spatial maps
        self._create_single_aggregated_spatial_maps("baseline")

    def _create_single_aggregated_spatial_maps(self, map_type: str) -> None:
        """Create aggregated spatial maps for a single type (model or baseline)."""
        logger.info(f"Creating aggregated {map_type} spatial maps...")

        # Combine all spatial metrics
        all_spatial_metrics = []
        spatial_key = (
            "spatial_metrics" if map_type == "model" else "baseline_spatial_metrics"
        )

        for results in self.file_results.values():
            if results and spatial_key in results and results[spatial_key] is not None:
                spatial_df = results[spatial_key].copy()
                spatial_df["file_name"] = results["file_name"]
                all_spatial_metrics.append(spatial_df)

        if not all_spatial_metrics:
            logger.warning(f"No {map_type} spatial metrics available for aggregation")
            return
        ...
        if (
            "latitude" in combined_spatial.columns
            and "longitude" in combined_spatial.columns
        ):
            coord_cols = ["latitude", "longitude"]
        ...
        aggregated = (
            combined_spatial.groupby(coord_cols)
            .agg(
                {
                    "bias": "mean",
                    "mae": "mean",
        ...
        for metric in self.metrics_to_plot:
            ...
            save_path = (
                self.output_dir
                / "spatial_maps"
                / f"aggregated_{map_type}_{metric}_map.png"
            )
            title = f"Aggregated {metric.upper()} - All 2023 Files ({map_type})"
    ...
    def save_results(self) -> None:
        ...
        for file_path, results in self.file_results.items():
            if results and "spatial_metrics" in results:
                ...
                if spatial_metrics is not None and not spatial_metrics.empty:
                    ...
                    logger.info(
                        f"Saved spatial metrics: {spatial_file} ({len(spatial_metrics)} rows)"
                    )
                else:
        ...
        if self.s3_results_saver and self.s3_results_saver.enabled:
            ...
            upload_results = self.s3_results_saver.upload_directory(
                str(self.output_dir), experiment_name=experiment_name
            )
    ...
    def run_evaluation(
        ...
        if self.parallel_enabled and self.n_workers > 1:
            ...
            with tqdm(
                ...
                for i, file_path in enumerate(data_files, 1):
                    file_name = Path(file_path).name
                    logger.info(
                        f"🔄 Processing file {i}/{len(data_files)}: {file_name}"
                    )
                    ...
                    try:
                        ...
                        if results:
                            ...
                            save_plots = self.config.get("output", {}).get(
                                "save_plots", True
                            )
                            if save_plots:
        ...
        logger.info(
            f"Successfully evaluated {successful_evaluations}/{len(data_files)} files"
        )
        ...
        if save_plots:
            ...
            logger.info(
                "Skipping aggregated spatial maps (save_plots disabled in config)"
            )
    ...
    def compute_aggregated_results(self) -> None:
        ...
        for results in self.file_results.values():
            if results and "metrics" in results:
                ...
                if (
                    "baseline_metrics" in results
                    and results["baseline_metrics"] is not None
                ):
        ...
        # Compute mean baseline metrics if available
        ...
        # Aggregate baseline spatial metrics across files
        all_baseline_spatial_metrics = []
        for results in self.file_results.values():
            if (
                results
                and "baseline_spatial_metrics" in results
                and results["baseline_spatial_metrics"] is not None
            ):
                all_baseline_spatial_metrics.append(results["baseline_spatial_metrics"])

        aggregated_baseline_spatial_metrics = None
        if all_baseline_spatial_metrics:
            # Combine all baseline spatial metrics DataFrames
            combined_baseline_spatial = pd.concat(
                all_baseline_spatial_metrics, ignore_index=True
            )

            # Group by coordinates and compute mean metrics
            coord_cols = (
                ["lat", "lon"]
                if "lat" in combined_baseline_spatial.columns
                else ["latitude", "longitude"]
            )
            aggregated_baseline_spatial_metrics = (
                combined_baseline_spatial.groupby(coord_cols)
                .agg(
                    {
                        "rmse": "mean",
                        "mae": "mean",
        ...
        self.aggregated_results = {
            ...
            "aggregated_baseline_spatial_metrics": aggregated_baseline_spatial_metrics.to_dict(
                "records"
            )
            if aggregated_baseline_spatial_metrics is not None
            else None,
            "file_summary": {
                file_name: {
                    "n_samples": results["n_samples"],
                    ...
                    "baseline_rmse": results.get("baseline_metrics", {}).get(
                        "rmse", None
                    ),
                    "baseline_mae": results.get("baseline_metrics", {}).get(
                        "mae", None
                    ),
                    "baseline_bias": results.get("baseline_metrics", {}).get(
                        "bias", None
                    ),
                }
                for file_name, results in self.file_results.items()
    ...
    def compute_temporal_analysis(self) -> None:
        ...
        monthly_metrics = (
            temporal_df.groupby(["year", "month", "month_name"])
            .agg(
                {
                    "rmse": "mean",
                    "mae": "mean",
        ...
        seasonal_metrics = (
            temporal_df.groupby(["year", "season"])
            .agg(
                {
                    "rmse": "mean",
                    "mae": "mean",
        ...
        save_plots = self.config.get("output", {}).get("save_plots", True)
        if save_plots:
            self.create_temporal_plots(
                temporal_df, daily_metrics, monthly_metrics, seasonal_metrics
            )
        else:
    ...
    def create_temporal_plots(
        ...
        for metric in self.metrics_to_plot:
            ...
            x_labels = [
                ...
                for _, row in monthly_metrics_sorted.iterrows()
            ]
            plt.plot(
                range(len(monthly_metrics_sorted)),
                monthly_metrics_sorted[metric],
                marker="o",
                linewidth=2,
                markersize=4,
            )
        ...
        for metric in self.metrics_to_plot:
            ...
            seasonal_pivot = seasonal_metrics.pivot(
                index="season", columns="year", values=metric
            )
    ...
    def _compute_comprehensive_spatial_metrics(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        try:
            ...
            # Calculate SNR and SNR_db using the correct residuals-based approach
            # Convert to pandas for complex calculations, then back to polars
            spatial_metrics_pd = spatial_metrics.to_pandas()

            # Calculate SNR using residuals-based approach (same as main metrics)
            # SNR = signal_power / noise_power
            # Signal: variance of true values
            # Noise: variance of residuals (prediction errors)

            # We need to calculate residuals for each spatial location
            # Since we don't have individual residuals per location in the aggregated data,
            # we'll use a simplified approach based on RMSE and variance

            # For spatial metrics, we can approximate SNR using:
            # SNR ≈ var_true / (RMSE^2 - bias^2) when bias is small
            # This is a reasonable approximation for spatial aggregation
            #
            # Mathematical relationship:
            # RMSE^2 = mean((y_true - y_pred)^2) = mean(residuals^2)
            # bias^2 = (mean(y_pred - y_true))^2
            # var(residuals) ≈ RMSE^2 - bias^2 (when residuals are approximately normal)
            # SNR = var(signal) / var(noise) = var_true / var(residuals)

            # Calculate noise power approximation: RMSE^2 - bias^2
            # This gives us the variance of residuals (noise power)
            noise_power_approx = (
                spatial_metrics_pd["rmse"] ** 2 - spatial_metrics_pd["bias"] ** 2
            )

            # Avoid negative noise power (can happen with large bias)
            noise_power_approx = np.maximum(noise_power_approx, 1e-10)

            # Calculate SNR: signal_power / noise_power
            spatial_metrics_pd["snr"] = (
                spatial_metrics_pd["var_true"] / noise_power_approx
            )

            # Log some statistics for debugging
            logger.debug("SNR calculation stats:")
            logger.debug(
                f"  Mean var_true: {spatial_metrics_pd['var_true'].mean():.6f}"
            )
            logger.debug(f"  Mean noise_power: {noise_power_approx.mean():.6f}")
            logger.debug(f"  Mean SNR: {spatial_metrics_pd['snr'].mean():.2f}")
            logger.debug(
                f"  SNR range: [{spatial_metrics_pd['snr'].min():.2f}, {spatial_metrics_pd['snr'].max():.2f}]"
            )

            # Handle edge cases
            spatial_metrics_pd["snr"] = spatial_metrics_pd["snr"].replace(
                [np.inf, -np.inf], float("inf")
            )
            spatial_metrics_pd["snr"] = spatial_metrics_pd["snr"].fillna(float("inf"))

            # Calculate SNR_db: 10 * log10(SNR)
            # Only calculate for finite SNR values
            finite_snr_mask = np.isfinite(spatial_metrics_pd["snr"])
            spatial_metrics_pd["snr_db"] = np.where(
                finite_snr_mask, 10 * np.log10(spatial_metrics_pd["snr"]), float("inf")
            )

            # Handle edge cases for SNR_db
            spatial_metrics_pd["snr_db"] = spatial_metrics_pd["snr_db"].replace(
                [np.inf, -np.inf], float("inf")
            )
            spatial_metrics_pd["snr_db"] = spatial_metrics_pd["snr_db"].fillna(
                float("inf")
            )
    ...
    def _extract_date_from_filename(
        self, filename: str
    ) -> Dict[str, Any]:  # src.data_engineering.split
        """
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Model evaluation with spatial analysis"
    )
    ...
    evaluator = ModelEvaluator(
        model_path=model_path, output_dir=output_dir, config=config
    )
    ...
    try:
        ...
        if results.get("mean_baseline_metrics"):
            ...
            print(
                f"📊 Baseline Pearson: {results['mean_baseline_metrics']['pearson']:.4f}"
            )

            # Show improvement
            rmse_improvement = (
                results["mean_baseline_metrics"]["rmse"]
                - results["mean_metrics"]["rmse"]
            )
            mae_improvement = (
                results["mean_baseline_metrics"]["mae"] - results["mean_metrics"]["mae"]
            )
