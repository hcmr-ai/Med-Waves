...
class ModelEvaluator:
    ...
    def load_and_preprocess_file(self, file_path: str, return_raw: bool = False) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, pd.DataFrame, int]:
        """
    ...
    def evaluate_file(self, file_path: str) -> Dict[str, Any]:
        ...
        X, y, regions, coords, metadata_df, vhm0_x_index = self.load_and_preprocess_file(file_path, return_raw=True)
        ...
        if vhm0_x_index is not None:
            ...
            baseline_spatial_metrics_df = self._compute_comprehensive_spatial_metrics(baseline_spatial_pl)
        ...
        results = {
            ...
            "baseline_spatial_metrics": baseline_spatial_metrics_df.to_pandas() if baseline_spatial_metrics_df is not None else None,
            "predictions": {
                "y_true": y,
                "y_pred": y_pred,
                "residuals": y_pred - y
        ...
        # Clean up memory
        del X, y, y_pred, spatial_df, spatial_pl, spatial_metrics_df, baseline_spatial_df, baseline_spatial_pl, baseline_spatial_metrics_df
    ...
    def create_spatial_maps(self, results: Dict[str, Any]) -> None:
        ...
        if baseline_spatial_metrics is not None:
            self._create_single_spatial_maps(file_name, baseline_spatial_metrics, "baseline")
    
    def _create_single_spatial_maps(self, file_name: str, spatial_metrics: pd.DataFrame, map_type: str) -> None:
        """
        ...
        for metric in self.metrics_to_plot:
            ...
            # Create spatial map
            save_path = self.output_dir / "spatial_maps" / f"{file_name}_{map_type}_{metric}_map.png"
    ...
    def _create_single_aggregated_spatial_maps(self, map_type: str) -> None:
        ...
        all_spatial_metrics = []
        spatial_key = "spatial_metrics" if map_type == "model" else "baseline_spatial_metrics"
        ...
        for metric in self.metrics_to_plot:
            ...
            # Create spatial map
            save_path = self.output_dir / "spatial_maps" / f"aggregated_{map_type}_{metric}_map.png"
    ...
    def compute_aggregated_results(self) -> None:
        ...
        for results in self.file_results.values():
            if results and "baseline_spatial_metrics" in results and results["baseline_spatial_metrics"] is not None:
        ...
        if all_baseline_spatial_metrics:
            ...
            combined_baseline_spatial = pd.concat(all_baseline_spatial_metrics, ignore_index=True)
            
            # Group by coordinates and compute mean metrics
            coord_cols = ["lat", "lon"] if "lat" in combined_baseline_spatial.columns else ["latitude", "longitude"]
            aggregated_baseline_spatial_metrics = combined_baseline_spatial.groupby(coord_cols).agg({
                "rmse": "mean",
                "mae": "mean", 
                "bias": "mean",
                "pearson": "mean",
        ...
        self.aggregated_results = {
            ...
            "aggregated_baseline_spatial_metrics": aggregated_baseline_spatial_metrics.to_dict('records') if aggregated_baseline_spatial_metrics is not None else None,
            "file_summary": {
                file_name: {
                    "n_samples": results["n_samples"],
    ...
    def _compute_comprehensive_spatial_metrics(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        try:
            ...
            noise_power_approx = spatial_metrics_pd['rmse']**2 - spatial_metrics_pd['bias']**2
            ...
            spatial_metrics_pd['snr'] = spatial_metrics_pd['var_true'] / noise_power_approx
            
            # Log some statistics for debugging
            logger.debug(f"SNR calculation stats:")
            logger.debug(f"  Mean var_true: {spatial_metrics_pd['var_true'].mean():.6f}")
            logger.debug(f"  Mean noise_power: {noise_power_approx.mean():.6f}")
            logger.debug(f"  Mean SNR: {spatial_metrics_pd['snr'].mean():.2f}")
            logger.debug(f"  SNR range: [{spatial_metrics_pd['snr'].min():.2f}, {spatial_metrics_pd['snr'].max():.2f}]")
            ...
            spatial_metrics_pd['snr_db'] = np.where(
                finite_snr_mask,
                10 * np.log10(spatial_metrics_pd['snr']),
                float('inf')
            )
