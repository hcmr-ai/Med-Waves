    ...
    try:
        ...
        if "WAVEAN" in filename:
            ...
            if len(date_str) >= 8:
                ...
                return {"year": year, "month": month, "day": day, "season": season}
    except Exception:
        pass
...
def _evaluate_single_file_worker(
...
class RobustModelEvaluator:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        (self.output_dir / "diagnostic_plots" / "spatial_maps" / "seasonal").mkdir(
            exist_ok=True
        )
        (self.output_dir / "diagnostic_plots" / "spatial_maps" / "aggregated").mkdir(
            exist_ok=True
        )
        ...
        logger.info(
            f"Initialized RobustModelEvaluator with output directory: {self.output_dir}"
        )
        logger.info(
            f"Parallel processing: {'enabled' if self.use_parallel else 'disabled'} (max_workers={self.max_workers})"
        )
    ...
    def _configure_dataloader_parallel_processing(self) -> None:
        ...
        logger.info(
            f"DataLoader parallel processing: {'enabled' if max_workers > 1 else 'disabled'} (max_workers={max_workers})"
        )
    ...
    def load_model(self, model_path: str) -> None:
        ...
        if (
            ...
            and self.s3_model_loader._is_s3_path(model_path)
        ):
            ...
            self.feature_names = components.get(
                "feature_names"
            )  # Note: S3ModelLoader uses 'feature_columns'
            ...
            if self.regional_scaler:
                logger.info(
                    f"Loaded regional scaler from S3: {type(self.regional_scaler)}"
                )
            else:
            ...
            if self.feature_names:
                logger.info(
                    f"Loaded feature names from S3: {len(self.feature_names)} features"
                )
            ...
            if feature_names_file.exists():
                ...
                logger.info(
                    f"Model expects {len(self.feature_names)} features: {self.feature_names}"
                )
            else:
    ...
    def get_data_files(self, data_path: str) -> List[str]:
        ...
        from src.pipelines.training.train_full_dataset import (
            get_data_files as training_get_data_files,
        )
    ...
    def load_and_preprocess_file(
        ...
        np.ndarray, np.ndarray, np.ndarray, np.ndarray, pd.DataFrame, int, np.ndarray
    ]:
        """
        ...
        try:
            ...
            X_raw, y, regions, coords = self.feature_engineer.prepare_features(
                combined_df, self.target_column
            )
            ...
            feature_name_to_idx = {
                name: idx for idx, name in enumerate(feature_engineer_names)
            }
            ...
            logger.info(
                f"Using {len(model_feature_indices)} features: {self.feature_names}"
            )
            ...
            metadata_df = pl.DataFrame(
                {"lat": coords[:, 0], "lon": coords[:, 1], "region": regions}
            )
    ...
    def _calculate_sea_bin_metrics(
        ...
        for bin_name, (min_val, max_val) in sea_bins.items():
            ...
            if np.sum(mask) > 0:
                ...
                bin_metrics["description"] = (
                    f"{bin_name.title()} seas ({min_val}-{max_val if max_val != float('inf') else '∞'}m)"
                )
    ...
    def create_all_spatial_maps(self) -> None:
        ...
        logger.info(
            f"Combined data from {len(all_data)} files: {len(combined_df)} total samples"
        )
        ...
        model_spatial_df = combined_df.select(
            ["lat", "lon", "y_true", "y_pred"]
        ).with_columns((pl.col("y_pred") - pl.col("y_true")).alias("residual"))

        baseline_spatial_df = (
            combined_df.select(["lat", "lon", "y_true", "vhm0_x"])
            .with_columns((pl.col("vhm0_x") - pl.col("y_true")).alias("residual"))
        ...
        logger.info(
            f"Calculated spatial metrics for {len(model_spatial_metrics)} locations"
        )

        # Create seasonal maps and get seasonal metrics
        seasonal_metrics = self._create_seasonal_maps_from_metrics(
            combined_df, model_spatial_metrics, baseline_spatial_metrics
        )

        # Create aggregated maps
        self._create_aggregated_maps_from_metrics(
            model_spatial_metrics, baseline_spatial_metrics
        )

        # Save spatial metrics to files (reuse already calculated seasonal metrics)
        self._save_spatial_metrics(
            model_spatial_metrics, baseline_spatial_metrics, seasonal_metrics
        )

        logger.info("✅ Created all spatial maps and saved spatial metrics")

    def _create_seasonal_maps_from_metrics(
        self, combined_df, model_spatial_metrics, baseline_spatial_metrics
        ...
        for season in seasons:
            ...
            if is_polars:
                ...
                model_season_metrics = model_spatial_metrics.join(
                    season_coords, on=["lat", "lon"], how="inner"
                )
                baseline_season_metrics = baseline_spatial_metrics.join(
                    season_coords, on=["lat", "lon"], how="inner"
                )
            else:
                ...
                model_season_metrics = model_spatial_metrics.merge(
                    season_coords, on=["lat", "lon"], how="inner"
                )
                baseline_season_metrics = baseline_spatial_metrics.merge(
                    season_coords, on=["lat", "lon"], how="inner"
                )
            ...
            metrics_to_plot = self.config.get("plots", {}).get(
    ...
    def _create_aggregated_maps_from_metrics(
        self, model_spatial_metrics, baseline_spatial_metrics
    ) -> None:
        ...
        # Create aggregated maps
        output_dir = (
            self.output_dir / "diagnostic_plots" / "spatial_maps" / "aggregated"
        )
        output_dir.mkdir(parents=True, exist_ok=True)
    ...
    def create_seasonal_diagnostic_plots(self) -> None:
        ...
        for season in seasons:
            ...
            season_output_dir = (
                self.output_dir / "diagnostic_plots" / "seasonal" / season
            )
            season_output_dir.mkdir(parents=True, exist_ok=True)

            # Create mock trainer for this season
            mock_trainer = self._create_seasonal_mock_trainer(
                season_data, season, self.config
            )
            ...
            if isinstance(season_data, pl.DataFrame):
                season_plotter.create_diagnostic_plots(
                    mock_trainer, season_data["y_pred"].to_numpy()
                )
            else:
                season_plotter.create_diagnostic_plots(
                    mock_trainer, season_data["y_pred"].values
                )
    ...
    def create_aggregated_diagnostic_plots(self) -> None:
        ...
        if isinstance(combined_df, pl.DataFrame):
            aggregated_plotter.create_diagnostic_plots(
                mock_trainer, combined_df["y_pred"].to_numpy()
            )
        else:
            aggregated_plotter.create_diagnostic_plots(
                mock_trainer, combined_df["y_pred"].values
            )

        logger.info("✅ Created overall aggregated diagnostic plots")

    def _create_seasonal_mock_trainer(
        self, season_data: pd.DataFrame, season: str, config: dict
    ):
        ...
        if isinstance(season_data, pl.DataFrame):
            baseline_regional_metrics = self._calculate_regional_metrics(
                y_true, season_data["vhm0_x"].to_numpy(), regions
            )
        else:
            baseline_regional_metrics = self._calculate_regional_metrics(
                y_true, season_data["vhm0_x"].values, regions
            )
        ...
        if isinstance(season_data, pl.DataFrame):
            baseline_sea_bin_metrics = self._calculate_sea_bin_metrics(
                y_true, season_data["vhm0_x"].to_numpy()
            )
        else:
            baseline_sea_bin_metrics = self._calculate_sea_bin_metrics(
                y_true, season_data["vhm0_x"].values
            )
        ...
        class SeasonalMockTrainer:
            def __init__(
                ...
                metrics,
                regional_metrics,
                baseline_regional_metrics,
                sea_bin_metrics,
                baseline_sea_bin_metrics,
                season,
            ):
                self.X_train = None
                self.y_train = None
                ...
                if isinstance(season_data, pl.DataFrame):
                    ...
                    self.vhm0_x_test = season_data[
                        "vhm0_x"
                    ].to_numpy()  # Add baseline data
                else:
    ...
    def _create_aggregated_mock_trainer(self, combined_df: pd.DataFrame, config: dict):
        ...
        if isinstance(combined_df, pl.DataFrame):
            baseline_regional_metrics = self._calculate_regional_metrics(
                y_true, combined_df["vhm0_x"].to_numpy(), regions
            )
        else:
            baseline_regional_metrics = self._calculate_regional_metrics(
                y_true, combined_df["vhm0_x"].values, regions
            )
        ...
        if isinstance(combined_df, pl.DataFrame):
            baseline_sea_bin_metrics = self._calculate_sea_bin_metrics(
                y_true, combined_df["vhm0_x"].to_numpy()
            )
        else:
            baseline_sea_bin_metrics = self._calculate_sea_bin_metrics(
                y_true, combined_df["vhm0_x"].values
            )
    ...
    def _create_single_spatial_map(
        self,
        spatial_metrics,
        metric: str,
        filename: str,
    ...
    def evaluate_file(self, file_path: str) -> Dict[str, Any]:
        ...
        X, y, regions, coords, metadata_df, vhm0_x_index, X_raw = (
            self.load_and_preprocess_file(file_path)
        )
        ...
        results = {
            ...
            "vhm0_x": vhm0_x
            if vhm0_x is not None
            else X_raw,  # Include raw features for spatial maps
        }
    ...
    def _evaluate_files_parallel(self, data_files: List[str]) -> None:
        ...
        logger.info(
            f"Evaluating {len(data_files)} files in parallel with {self.max_workers} workers..."
        )
        ...
        with ProcessPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            future_to_file = {
                executor.submit(
                    _evaluate_single_file_worker, (file_path, model_path, self.config)
                ): file_path
            ...
            for future in tqdm(
                ...
                if result is not None:
                    ...
                    if successful_files <= 3:
                        logger.info(
                            f"File {successful_files}: {result['file_name']} evaluated successfully"
                        )
                else:
                    failed_files += 1
        ...
        logger.info(
            f"✅ Parallel evaluation completed: {successful_files} successful, {failed_files} failed"
        )
    ...
    def _evaluate_files_sequential(self, data_files: List[str]) -> None:
        ...
        logger.info(
            f"✅ Sequential evaluation completed: {len(self.file_results)} files processed"
        )
    ...
    def compute_aggregated_results(self) -> None:
        ...
        if all_baseline_metrics:
            mean_baseline_metrics = self._compute_combined_metrics(
                self.file_results, "baseline"
            )
    ...
    def _compute_combined_metrics(
        self, file_results: Dict, metric_type: str
    ) -> Dict[str, float]:
        """
    ...
    def _save_spatial_metrics(
        self,
        model_spatial_metrics,
        baseline_spatial_metrics,
        seasonal_metrics: Dict[str, Dict[str, Any]],
    ) -> None:
    ...
    def _save_seasonal_spatial_metrics(
        self, seasonal_metrics: Dict[str, Dict[str, Any]], output_dir: Path
    ) -> None:
        ...
        for season in seasons:
            ...
            baseline_season_file = (
                seasonal_dir / f"{season}_baseline_spatial_metrics.csv"
            )
            ...
            logger.info(
                f"Saved {season} baseline spatial metrics: {baseline_season_file}"
            )
    ...
    def save_results_to_s3(self) -> None:
        ...
        results = {
            ...
            "evaluation_summary": {
                "total_files_processed": len(self.file_results),
                "total_samples": sum(
                    result.get("n_samples", 0) for result in self.file_results.values()
                ),
                "evaluation_timestamp": datetime.now().isoformat(),
            },
        }
        ...
        logger.info(
            f"✅ All evaluation results uploaded to S3: {self.s3_results_saver.prefix}"
        )
                ...
                for file_name, file_data in value.items():
                    ...
                    for metric_key, metric_value in file_data.items():
                        if metric_value is not None and hasattr(metric_value, "item"):
                            serializable_aggregated[key][file_name][metric_key] = (
                                metric_value.item()
                            )
                        else:
                            serializable_aggregated[key][file_name][metric_key] = (
        ...
        for file_name, results in self.file_results.items():
            if results:
                serializable_file_results[file_name] = {
                    ...
                    "baseline_metrics": {
                        k: (v.item() if hasattr(v, "item") else float(v))
                        for k, v in results.get("baseline_metrics", {}).items()
                    }
                    if results.get("baseline_metrics")
                    else None,
                }
    ...
    try:
        ...
        if evaluator.aggregated_results:
            ...
            if results.get("mean_baseline_metrics"):
                print(
                    f"📊 Baseline RMSE: {results['mean_baseline_metrics']['rmse']:.4f}"
                )
                print(f"📊 Baseline MAE: {results['mean_baseline_metrics']['mae']:.4f}")
                print(
                    f"📊 Baseline Bias: {results['mean_baseline_metrics']['bias']:.4f}"
                )
                print(
                ...
                mae_improvement = (
                    results["mean_baseline_metrics"]["mae"]
                    - results["mean_metrics"]["mae"]
                )
