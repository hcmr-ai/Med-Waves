...
def sample_file(
    file_path: Path, feature_cols: list[str], frac: float = 0.01
) -> pl.DataFrame:
    df = pl.read_parquet(file_path).select(feature_cols)
    n_rows = int(len(df) * frac)
    if n_rows == 0:
        return pl.DataFrame(
            schema={col: df[col].dtype for col in feature_cols}
        )  # Return empty frame
    return df.sample(n=n_rows)


def collect_samples(
    parquet_dir: Path, feature_cols: list[str], glob_pattern: str, frac: float = 0.01
) -> pl.DataFrame:
    all_samples = []
...
def compute_and_plot_correlation(
    df: pl.DataFrame, save_path: Path, method: str = "pearson"
):
    df_pd = df.to_pandas()
...
def main():
    ...
    output_dir = Path(
        f"/data/tsolis/AI_project/output/eda/augmented_with_labels/{year}"
    )
    output_dir.mkdir(parents=True, exist_ok=True)

    sampled_df = collect_samples(
        parquet_dir, feature_cols, f"WAVEAN{year}*.parquet", frac=frac
    )
