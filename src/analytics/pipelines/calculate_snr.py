...
def compute_local_snr_per_pixel(
    ground_truth: np.ndarray, reference: np.ndarray, patch_size: int = 5
) -> np.ndarray:
    """
    ...
    ground_truth_pad = np.pad(
        ground_truth,
        ((0, 0), (pad, pad), (pad, pad)),
        mode="constant",
        constant_values=np.nan,
    )
    residuals_pad = np.pad(
        residuals,
        ((0, 0), (pad, pad), (pad, pad)),
        mode="constant",
        constant_values=np.nan,
...
def compute_correlations(
    ...
    valid_mask = (
        np.isfinite(ground_truth) & np.isfinite(reference) & np.isfinite(snr_per_pixel)
    )
    ground_truth = ground_truth[valid_mask]
    reference = reference[valid_mask]
    ...
    for bin_name, metrics in bin_results.items():
        ...
        valid_bin_mask = (
            np.isfinite(error_mag) & np.isfinite(gt_bin) & np.isfinite(ref_bin)
        )
        ...
        valid_snr_mask = (
            np.isfinite(local_snr) & np.isfinite(error_mag) & (local_snr > 0)
        )
...
def plot_snr_analysis(
    ...
    [bin_results[b]["mae"] for b in bin_names]
    ...
    ax1 = axes[0, 0]
    colors = [
        "#2E7D32" if snr > 10 else "#F57C00" if snr > 5 else "#C62828"
    ...
    for i, (v, n) in enumerate(zip(snr_db_values, n_samples, strict=False)):
        if np.isfinite(v):
            ax1.text(
                i,
                v + 0.5,
    ...
    sc2 = ax2.scatter(
        rmse_values,
        snr_db_values,
        s=[n / 100 for n in n_samples],
        c=range(len(bin_names)),
        cmap="viridis",
        alpha=0.7,
        edgecolors="black",
        linewidth=1.5,
    ...
    for _, (x, y, bn) in enumerate(
        zip(rmse_values, snr_db_values, bin_names, strict=False)
    ):
    ...
    if len(valid_idx) > 2:
        ...
        ax2.text(
            0.05,
            0.95,
            f"Correlation: {corr_rmse_snr:.3f}",
    ...
    ax3 = axes[1, 0]
    colors_corr = [
        "#1976D2" if abs(c) > 0.6 else "#757575"
        for c in bin_corr_values
        if np.isfinite(c)
    ]
    valid_corr_bins = [
        bn for bn, c in zip(bin_names, bin_corr_values, strict=False) if np.isfinite(c)
    ]
    valid_corr_values = [c for c in bin_corr_values if np.isfinite(c)]

    if len(valid_corr_values) > 0:
        ax3.bar(
            range(len(valid_corr_bins)),
            valid_corr_values,
            color=colors_corr,
            alpha=0.7,
            edgecolor="black",
    ...
    ax3.axhline(
        ...
        linewidth=1,
        alpha=0.5,
        label="Strong correlation (|r|>0.6)",
    )
    ...
    ax4.set_title(
        f"Plot 4: Error vs SNR (All Pixels, n={len(error_plot):,})",
        fontweight="bold",
        pad=10,
    )
        ...
        if error_var < 1e-10 or snr_var < 1e-10:
            ...
            if np.isfinite(overall_pixel_corr) and np.isfinite(overall_pixel_pval):
                ...
                interpretation = (
                    ...
                    if abs(overall_pixel_corr) > 0.6
                    else "Moderate/weak correlation suggests other\n"
...
def print_detailed_table(bin_results: Dict, correlations: Dict):
    ...
    print("=" * 120)
    print(
        f"{'Bin':<12} {'N_samples':<12} {'Signal_Var':<12} {'Noise_Var':<12} "
...
def analyze_snr_complete(
    ...
    valid_mask = (
        np.isfinite(gt_flat) & np.isfinite(ref_flat) & (gt_flat >= 0) & (ref_flat >= 0)
    )
    ...
    if len(original_shape) == 3:
        ...
        snr_per_pixel_full = compute_local_snr_per_pixel(
            ground_truth, reference, patch_size=5
        )
    ...
    plot_snr_analysis(
        bin_results, correlations, gt_flat, ref_flat, snr_per_pixel, output_path
    )
...
def spatial_snr_analysis(
    ...
    # Create spatial grid
    lat_bins = np.arange(
        lat_valid.min(), lat_valid.max() + grid_resolution, grid_resolution
    )
    lon_bins = np.arange(
        lon_valid.min(), lon_valid.max() + grid_resolution, grid_resolution
    )

    print(
        f"Grid resolution: {grid_resolution}° ({len(lat_bins) - 1}x{len(lon_bins) - 1} cells)"
    )
    ...
    if np.sum(np.isfinite(corr_grid)) > 0:
        ...
        print(
            f"Correlation range: [{np.nanmin(corr_grid):.3f}, {np.nanmax(corr_grid):.3f}]"
        )
    print("=" * 80)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="SNR Analysis for Wave Model Evaluation"
    )
    parser.add_argument(
        "--input-dir",
        type=str,
    ...
    parser.add_argument("--year", type=int, default=2022, help="Year to analyze")
    parser.add_argument(
        "--ground-truth-col",
        type=str,
    ...
    parser.add_argument(
        "--min-samples", type=int, default=5, help="Minimum samples per bin"
    )
    parser.add_argument(
        "--spatial-analysis",
        action="store_true",
    ...
    if args.spatial_analysis:
        print(
            f"  2. Review the spatial analysis: {args.output_path.replace('.png', '_spatial.png')}"
        )
    print("  3. Check correlation values in the table above")
    print(
        "  4. If correlation > 0.6, consider adding SNR as input feature to TransUNet"
    )
