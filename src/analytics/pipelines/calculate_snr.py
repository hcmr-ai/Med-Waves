...
def plot_snr_analysis(
    ...
    for _, (x, y, bn) in enumerate(zip(rmse_values, snr_db_values, bin_names, strict=False)):
    ...
    valid_corr_bins = [bn for bn, c in zip(bin_names, bin_corr_values, strict=False) if np.isfinite(c)]
    valid_corr_values = [c for c in bin_corr_values if np.isfinite(c)]

    if len(valid_corr_values) > 0:
        ax3.bar(range(len(valid_corr_bins)), valid_corr_values,
