...
def compute_descriptive_stats(
    df: pl.DataFrame, feature_cols: list[str], save_path: Path
):
...
def plot_pairplot(df: pl.DataFrame, feature_cols: list[str], save_path: Path):
    ...
    pairplot = sns.pairplot(
        df.to_pandas(), corner=True, plot_kws={"alpha": 0.3, "s": 10}
    )
...
def plot_correlation_heatmap(
    df: pl.DataFrame, feature_cols: list[str], save_path: Path, method="pearson"
):
...
def main():
    ...
    for year in years:
        main_parquet_data_path = (
            f"/data/tsolis/AI_project/parquet/{data_origin}/hourly/"
        )
