...
def analyze_nan_patterns(df: pl.DataFrame) -> Dict:
    ...
    logger.info(
        f"Valid data points: {len(valid_data):,} ({len(valid_data) / len(df) * 100:.1f}%)"
    )
    logger.info(
        f"Invalid data points: {len(invalid_data):,} ({len(invalid_data) / len(df) * 100:.1f}%)"
    )
...
def analyze_regional_nan_patterns(
    valid_data: pl.DataFrame, invalid_data: pl.DataFrame
) -> Dict:
    ...
    valid_region_counts = (
        valid_data.group_by("region")
        .agg([pl.len().alias("valid_count")])
        .sort("valid_count", descending=True)
    )

    # Count invalid data by region
    invalid_region_counts = (
        invalid_data.group_by("region")
        .agg([pl.len().alias("invalid_count")])
...
def create_nan_visualizations(
    df: pl.DataFrame, analysis_results: Dict, output_dir: str = "nan_analysis"
):
    ...
    for bar, pct in zip(bars1, nan_percentages, strict=False):
    ...
    # Add percentage labels on bars
    for bar, pct in zip(bars2, valid_percentages, strict=False):
        height = bar.get_height()
        ax2.text(
            bar.get_x() + bar.get_width() / 2.0,
            height + 1,
    ...
    # Plot valid data
    ax1.scatter(
        valid_sample["longitude"].to_list(),
        valid_sample["latitude"].to_list(),
        c="green",
        alpha=0.6,
        s=1,
        label=f"Valid Data ({len(valid_data):,} points)",
    )
    ax1.set_xlabel("Longitude")
    ax1.set_ylabel("Latitude")
    ax1.set_title(
        "Spatial Distribution of Valid Data (Sea Areas)", fontsize=14, fontweight="bold"
    )
    ...
    # Plot invalid data
    ax2.scatter(
        invalid_sample["longitude"].to_list(),
        invalid_sample["latitude"].to_list(),
        c="red",
        alpha=0.6,
        s=1,
        label=f"Invalid Data ({len(invalid_data):,} points)",
    )
    ax2.set_xlabel("Longitude")
    ax2.set_ylabel("Latitude")
    ax2.set_title(
        "Spatial Distribution of Invalid Data (Land Areas)",
        fontsize=14,
        fontweight="bold",
    )
    ...
    bars1 = ax1.bar(
        valid_regions, valid_counts, color=["#1f77b4", "#ff7f0e", "#2ca02c"]
    )
    ...
    for bar, count, pct in zip(bars1, valid_counts, valid_percentages, strict=False):
        height = bar.get_height()
        ax1.text(
            bar.get_x() + bar.get_width() / 2.0,
            height + height * 0.01,
    ...
    bars2 = ax2.bar(
        invalid_regions, invalid_counts, color=["#1f77b4", "#ff7f0e", "#2ca02c"]
    )
    ...
    plt.tight_layout()
    plt.savefig(
        output_path / "regional_valid_invalid.png", dpi=300, bbox_inches="tight"
    )
    plt.close()
...
def print_nan_analysis_summary(analysis_results: Dict):
    ...
    logger.info(
        f"Valid data points (sea areas): {len(valid_data):,} ({len(valid_data) / total_points * 100:.1f}%)"
    )
    logger.info(
        f"Invalid data points (land areas): {len(invalid_data):,} ({len(invalid_data) / total_points * 100:.1f}%)"
    )
    logger.info("")
    ...
    for col, stats in nan_stats.items():
        logger.info(
            f"  {col:15} {stats['nan_percentage']:>6.1f}% NaN ({stats['valid_count']:>8,} valid)"
        )
    ...
    for row in regional_analysis["valid_region_counts"].iter_rows(named=True):
        logger.info(
            f"  {row['region'].title():15} {row['valid_count']:>8,} points ({row['valid_percentage']:>5.1f}%)"
        )
    ...
    for row in regional_analysis["invalid_region_counts"].iter_rows(named=True):
        logger.info(
            f"  {row['region'].title():15} {row['invalid_count']:>8,} points ({row['invalid_percentage']:>5.1f}%)"
        )
    ...
    if len(atlantic_valid) > 0:
        ...
        if atlantic_pct < 30:
            logger.warning(
                f"  ⚠️  Atlantic region has only {atlantic_pct:.1f}% of valid sea data"
            )
        else:
            logger.info(
                f"  ✅ Atlantic region has {atlantic_pct:.1f}% of valid sea data"
            )
    ...
    logger.info(
        "  • The NaN handling in your pipeline is correct (dropping land areas)"
    )
    logger.info("  • Focus on improving model regularization instead of NaN handling")
    logger.info(
        "  • Consider the temporal distribution shift as the main overfitting cause"
    )
...
def main():
    ...
    logger.info(
        "NaN pattern analysis complete! Check the 'nan_analysis' folder for visualizations."
    )
