...
def load_sample_data(file_paths: List[str], max_files: int = 3) -> pl.DataFrame:
    ...
    if original_size != final_size:
        logger.info(
            f"Removed {original_size - final_size:,} rows with NaN values ({((original_size - final_size) / original_size * 100):.1f}%)"
        )
...
def create_visualizations(
    df: pl.DataFrame, analysis_results: Dict, output_dir: str = "regional_analysis"
):
    ...
    wedges, texts, autotexts = ax2.pie(
        percentages, labels=regions, autopct="%1.1f%%", colors=colors, startangle=90
    )
    ...
    region_colors = {
        ...
        1: "#ff7f0e",
        2: "#2ca02c",
    }  # atlantic, mediterranean, eastern_med
    ...
    for region_id in regions:
        ...
        if len(region_data) > 0:
            ...
            ax.scatter(
                region_data["longitude"].to_list(),
                region_data["latitude"].to_list(),
                c=region_colors[region_id],
                label=f"{region_name} ({len(region_data):,} points)",
                alpha=0.6,
                s=1,
            )
    ...
    ax.axvline(
        x=-5, color="red", linestyle="--", alpha=0.7, label="Atlantic Boundary (lon=-5)"
    )
    ax.axvline(
        x=30,
        color="red",
        linestyle="--",
        alpha=0.7,
        label="Eastern Med Boundary (lon=30)",
    )

    ax.set_xlabel("Longitude", fontsize=12)
    ax.set_ylabel("Latitude", fontsize=12)
    ax.set_title(
        "Spatial Distribution of Data Points by Region", fontsize=14, fontweight="bold"
    )
    ...
    heatmap_data = temporal_pandas.pivot_table(
        index=["year", "month"], columns="region", values="count", fill_value=0
    )
    ...
    ax.set_title(
        "Data Points by Region and Time Period", fontsize=14, fontweight="bold"
    )
    ...
    ax.set_title(
        "Regional Distribution Summary", fontsize=14, fontweight="bold", pad=20
    )
...
def print_analysis_summary(analysis_results: Dict):
    ...
    for row in region_counts.iter_rows(named=True):
        logger.info(
            f"  {row['region'].title():15} {row['count']:>8,} points ({row['percentage']:>5.1f}%)"
        )
    ...
    if min_percentage < 5:
        logger.warning(
            f"  ⚠️  One region is underrepresented: {min_percentage:.1f}% of data"
        )
    ...
    if len(atlantic_data) > 0:
        ...
        if atlantic_pct < 10:
            logger.warning(
                f"  ⚠️  Atlantic region has only {atlantic_pct:.1f}% of data - this explains poor Atlantic performance!"
            )
        else:
...
def main():
    ...
    logger.info(
        "Analysis complete! Check the 'regional_analysis' folder for visualizations."
    )
