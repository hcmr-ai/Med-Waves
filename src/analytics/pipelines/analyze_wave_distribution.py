...
def create_distribution_plots(
    ...
    wedges, texts, autotexts = ax1.pie(
        bin_counts, labels=bin_names, autopct="%1.1f%%", colors=colors, startangle=90
    )
    ...
    ax3.bar(
        x - width / 2, region_counts, width, label="Total Samples", alpha=0.8
    )
    ax3.bar(
        x + width / 2,
        extreme_counts,
        width,
        label="Extreme Waves (>9m)",
        color="red",
        alpha=0.8,
    )
    ...
    if sum(extreme_counts) > 0:
        wedges, texts, autotexts = ax5.pie(
            extreme_counts, labels=region_names, autopct="%1.1f%%", startangle=90
        )
        ax5.set_title(
            "Extreme Waves by Region\n(>9m waves)", fontsize=14, fontweight="bold"
        )
    else:
        ax5.text(
            0.5,
            0.5,
            "No extreme waves found",
            ha="center",
            va="center",
    ...
    if total_year_month_bins:
        ...
        ax6.text(
            0.5,
            0.5,
            "No temporal-bin data available",
            ha="center",
            va="center",
        ...
        ax6.set_title(
            "Wave Height Distribution by Month", fontsize=14, fontweight="bold"
        )
...
def analyze_single_file(file_path: str) -> Dict:
    ...
    try:
        ...
        if wave_height_col is None:
            return {
                "file": file_path,
                "error": f"No wave height column found. Available: {df.columns}",
            }
        ...
        if "datetime" in df_clean.columns:
            ...
            year_month_data = df_clean.select(
                ["year", "month", wave_height_col]
            ).to_numpy()
            for year, month, wave_height in year_month_data:
                ...
                if year_int is not None and month_int is not None:
                    ...
                    if bin_name:
                        ...
                        year_month_bin_counts[bin_key] = (
                            year_month_bin_counts.get(bin_key, 0) + 1
                        )
    ...
    try:
        ...
        for result in all_results:
            ...
            for year_month_key, count in result.get("year_month", {}).items():
                total_year_month[year_month_key] = (
                    total_year_month.get(year_month_key, 0) + count
                )

            # Aggregate year-month-bin counts
            for year_month_bin_key, count in result.get("year_month_bins", {}).items():
                total_year_month_bins[year_month_bin_key] = (
                    total_year_month_bins.get(year_month_bin_key, 0) + count
                )
        ...
        for bin_name, (min_h, max_h) in WAVE_BINS.items():
            ...
            print(
                f"{bin_name:12} ({range_str:>8}): {count:10,} samples ({percentage:5.1f}%)"
            )
        ...
        for region_id in range(3):
            ...
            region_percentage = (
                (total_region / total_samples) * 100 if total_samples > 0 else 0
            )
            extreme_percentage = (
                (extreme_region / total_bins["extreme"]) * 100
                if total_bins["extreme"] > 0
                else 0
            )

            print(
                f"{region_names[region_id]:20}: {total_region:10,} samples ({region_percentage:5.1f}%)"
            )
            print(
                f"  └─ Extreme waves: {extreme_region:10,} samples ({extreme_percentage:5.1f}% of all extreme)"
            )
        ...
        if total_year_month_bins:
            ...
            for year_month in sorted(year_month_groups.keys()):
                ...
                for bin_name in WAVE_BINS.keys():
                    ...
                    percentage = (
                        (count / total_for_month) * 100 if total_for_month > 0 else 0
                    )
        ...
        if total_samples > 0:
            print(
                f"Percentage of total: {(total_bins['extreme'] / total_samples) * 100:.3f}%"
            )
        else:
        ...
        for region_id in range(3):
            ...
            if total_region > samples_per_region:
                ...
                print(
                    f"  Extreme waves: {extreme_region:,} → {extreme_region - extreme_loss:,} (lose {extreme_loss:,})"
                )

        print(f"\nTotal extreme waves potentially lost: {potential_loss:,}")
        if total_bins["extreme"] > 0:
            print(
                f"Percentage of extreme waves lost: {(potential_loss / total_bins['extreme']) * 100:.1f}%"
            )
        else:
