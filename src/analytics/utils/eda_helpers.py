...
def flag_missing(df: pl.DataFrame, value_col: str = "VHM0") -> pl.DataFrame:
    ...
    """
    return df.with_columns((pl.col(value_col).is_null()).alias("is_missing"))


def flag_outliers_zscore(
    df: pl.DataFrame, value_col: str = "VHM0", thresh: float = 3.0
) -> pl.DataFrame:
    """
    ...
    return df.with_columns(
        ((pl.col(value_col) - mu).abs() / sigma > thresh).alias("is_outlier")
    )


def smooth_rolling(
    df: pl.DataFrame, value_col: str = "VHM0", window: int = 7
) -> pl.DataFrame:
    """
...
def difference_series(df: pl.DataFrame, value_col: str = "VHM0") -> pl.DataFrame:
    ...
    """
    return df.with_columns(pl.col(value_col).diff().fill_null(0).alias("differenced"))
