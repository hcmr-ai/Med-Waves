    ...
    for bin_config in bins:
        ...
        if bin_count > 0:
            ...
            bin_metrics = evaluate_model(
                bin_y_pred, bin_y_true
            )  # Note: evaluate_model expects (y_pred, y_true)
            ...
            if enable_logging:
                logger.info(
                    f"{bin_name.title()} ({bin_description}) - Count: {bin_count:,} ({bin_metrics['percentage']:.1f}%), RMSE: {bin_metrics['rmse']:.4f}, MAE: {bin_metrics['mae']:.4f}"
                )
        else:
            if enable_logging:
                logger.info(
                    f"{bin_name.title()} ({bin_description}) - No samples in this range"
                )
