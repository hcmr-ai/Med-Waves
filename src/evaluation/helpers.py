...
class TrainingMetricsStorage:
    ...
    def _get_current_batch_from_files(self) -> int:
        ...
        for file_path in all_files:
            ...
            if filename.startswith("batch_metrics_") or filename.startswith(
                "validation_metrics_"
            ):
                try:
    ...
    def save_batch_metrics(self, metrics: Dict[str, Any]):
        ...
        if self.current_batch > self.max_history:
            old_file = (
                self.save_path
                / f"batch_metrics_{self.current_batch - self.max_history:06d}.parquet"
            )
            if old_file.exists():
                old_file.unlink()
    ...
    def save_feature_importance(self, importance_data: Dict[str, Any]):
        ...
        file_path = (
            self.save_path / f"feature_importance_{importance_data['batch']:06d}.csv"
        )
        df.to_csv(file_path, index=False)
