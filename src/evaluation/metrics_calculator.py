...
class MetricsCalculator:
    ...
    def calculate_sea_bin_metrics(
        self, y_true: np.ndarray, y_pred: np.ndarray, enable_logging: bool = False
    ) -> Dict[str, Dict[str, float]]:
        """
        ...
        return calculate_sea_bin_metrics(
            y_true, y_pred, sea_bin_config, enable_logging=enable_logging
        )
    ...
    def calculate_spatial_metrics(
        ...
        if coords is None or len(coords) == 0:
            self.logger.warning(
                "No coordinate information available for spatial metrics"
            )
            return {}
        ...
        for i, lat_center in enumerate(lat_grid):
            for j, lon_center in enumerate(lon_grid):
                ...
                lat_mask = (lats >= lat_center - grid_resolution / 2) & (
                    lats < lat_center + grid_resolution / 2
                )
                lon_mask = (lons >= lon_center - grid_resolution / 2) & (
                    lons < lon_center + grid_resolution / 2
                )
    ...
    def calculate_comprehensive_metrics(
        ...
        if regions is not None:
            metrics["regional"] = self.calculate_regional_metrics(
                y_true, y_pred, regions
            )

        # Sea-bin metrics
        metrics["sea_bin"] = self.calculate_sea_bin_metrics(
            y_true, y_pred, enable_logging
        )

        # Spatial metrics (if coordinates available)
        if coords is not None:
            grid_resolution = self.evaluation_config.get("spatial_grid_resolution", 1.0)
            metrics["spatial"] = self.calculate_spatial_metrics(
                y_true, y_pred, coords, grid_resolution
            )
            ...
            for bin_idx, bin_config in enumerate(bins):
                ...
                if len(bin_model_errors) > 0:
                    ...
                    model_abs_error_data_by_region_bin[region].append(
                        bin_model_abs_errors
                    )
                    model_rmse_data_by_region_bin[region].append(
                        np.sqrt(np.mean(bin_model_errors**2))
                    )
                ...
                if baseline_errors is not None:
                    ...
                    if len(bin_baseline_errors) > 0:
                        ...
                        baseline_abs_error_data_by_region_bin[region].append(
                            bin_baseline_abs_errors
                        )
                        baseline_rmse_data_by_region_bin[region].append(
                            np.sqrt(np.mean(bin_baseline_errors**2))
                        )
                        baseline_count_data_by_region_bin[region].append(
                            len(bin_baseline_errors)
                        )
                    else:
                        baseline_error_data_by_region_bin[region].append([])
                        baseline_abs_error_data_by_region_bin[region].append([])
    ...
    def log_metrics_summary(self, metrics: Dict[str, Any], stage: str = "Evaluation"):
        ...
        self.logger.info(
            f"  SNR: {basic.get('snr', 0):.1f} ({basic.get('snr_db', 0):.1f} dB)"
        )
        ...
        if "spatial" in metrics:
            ...
            self.logger.info(
                f"  Spatial analysis: {spatial['grid_shape']} grid, {spatial['grid_resolution']}° resolution"
            )
