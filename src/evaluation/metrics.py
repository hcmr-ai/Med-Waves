...
def evaluate_model_spatial(spatial_df):
    ...
    if is_polars:
        ...
        coord_cols = (
            ["lat", "lon"] if "lat" in spatial_df.columns else ["latitude", "longitude"]
        )
        ...
        spatial_metrics = spatial_metrics.with_columns(
            [
                ...
                (pl.col("residual_std").pow(2) + pl.col("residual_mean").pow(2))
                .sqrt()
                .alias("rmse"),
        ...
        for group in spatial_df.group_by(coord_cols):
            ...
            if len(group_data) >= 2:
                try:
                    corr = pearsonr(
                        group_data["y_true"].to_numpy(), group_data["y_pred"].to_numpy()
                    )[0]
        ...
        spatial_metrics.columns = [
            f"{col[0]}_{col[1]}" if col[1] else col[0]
            for col in spatial_metrics.columns
        ]

        # Calculate metrics using vectorized operations (much faster than apply)
        spatial_metrics["bias"] = (
            np.mean(spatial_metrics["y_pred_mean"]) - spatial_metrics["y_true_mean"]
        )
        spatial_metrics["mae"] = np.abs(spatial_metrics["residual_mean"])
        spatial_metrics["rmse"] = np.sqrt(
            spatial_metrics["residual_std"] ** 2 + spatial_metrics["residual_mean"] ** 2
        )
        spatial_metrics["diff"] = (
            spatial_metrics["y_pred_mean"] - spatial_metrics["y_true_mean"]
        )
        ...
        spatial_metrics["snr"] = np.where(
            noise_power > 0, signal_power / noise_power, np.inf
        )
        spatial_metrics["snr_db"] = np.where(
            np.isfinite(spatial_metrics["snr"]),
            10 * np.log10(spatial_metrics["snr"]),
            np.inf,
        )
        ...
        spatial_metrics["pearson"] = (
            spatial_df.groupby(coord_cols)
            .apply(calc_pearson_vectorized, include_groups=False)
            .values
        )
    ...
    for bin_config in sea_bins:
        ...
        if bin_count > 0:
            ...
            if bin_data["sum_baseline_mae"] > 0:
                ...
                pct_model_worse = (bin_data["count_model_worse"] / bin_count) * 100
...
def compute_snr(features: np.ndarray) -> np.ndarray:
    ...
    var_val = (
        features.var(axis=1) + 1e-8
    )  # Variance across channels (add epsilon to avoid division by zero)
