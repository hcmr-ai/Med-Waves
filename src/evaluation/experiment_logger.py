...
class ExperimentLogger:
    ...
    def _init_comet_experiment(self):
        ...
        try:
            if self.logging_config.get("use_comet", False) and Experiment is not None:
                ...
                experiment_name = self.config.get("output", {}).get(
                    "experiment_name", "full_dataset_training"
                )
                experiment_name = (
                    f"{datetime.now().strftime('%Y%m%d_%H%M')}_{experiment_name}"
                )
    ...
    def log_experiment_params(self):
        ...
        try:
            ...
            self.experiment.log_parameters(
                self.config.get("feature_block", {}), prefix="feature_block"
            )
            self.experiment.log_parameters(
                self.config.get("preprocessing", {}), prefix="preprocessing"
            )
            self.experiment.log_parameters(
                self.config.get("memory_monitoring", {}), prefix="parallel_processing"
            )
            self.experiment.log_parameters(
                self.config.get("logging", {}), prefix="output"
            )
            self.experiment.log_parameters(
                self.config.get("output", {}), prefix="output"
            )
            self.experiment.log_parameters(self.config.get("s3", {}), prefix="s3")
            # Log evaluation configuration
            self.experiment.log_parameters(
                self.config.get("evaluation", {}), prefix="evaluation"
            )

            # Log diagnostics configuration
            self.experiment.log_parameters(
                self.config.get("training_diagnostics", {}),
                prefix="training_diagnostics",
            )
    ...
    def log_dataset_info(self, dataset_info: Dict[str, Any]):
        ...
        try:
            ...
            self.experiment.log_metric(
                "dataset_total_features", dataset_info.get("total_features", 0)
            )
            self.experiment.log_metric(
                "dataset_files_processed", dataset_info.get("files_processed", 0)
            )
            ...
            if sampling_info:
                self.experiment.log_metric(
                    "sampling_max_per_file", sampling_info.get("max_per_file", 0)
                )
                self.experiment.log_metric(
                    "sampling_reduction_percent",
                    sampling_info.get("reduction_percent", 0),
                )
                self.experiment.log_parameter(
                    "sampling_strategy", sampling_info.get("strategy", "none")
                )
    ...
    def log_split_info(self, split_info: Dict[str, Any]):
        ...
        try:
            # Log split sizes
            self.experiment.log_metric(
                "split_train_samples", split_info.get("train_samples", 0)
            )
            self.experiment.log_metric(
                "split_val_samples", split_info.get("val_samples", 0)
            )
            self.experiment.log_metric(
                "split_test_samples", split_info.get("test_samples", 0)
            )

            # Log split ratios
            self.experiment.log_metric(
                "split_train_ratio", split_info.get("train_ratio", 0)
            )
            self.experiment.log_metric(
                "split_val_ratio", split_info.get("val_ratio", 0)
            )
            self.experiment.log_metric(
                "split_test_ratio", split_info.get("test_ratio", 0)
            )
            ...
            if spatial_maps_dir.exists():
                for spatial_file in spatial_maps_dir.glob("*.png"):
                    ...
                    self.experiment.log_image(
                        str(spatial_file), name=f"spatial_maps/{spatial_file.stem}"
                    )
    ...
    def log_model_artifacts(self, save_path: Path):
        ...
        try:
            ...
            self.experiment.log_asset_folder(
                str(save_path), log_file_name="model_artifacts"
            )
