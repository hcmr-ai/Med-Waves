...
class CorrectorComparisonPlotter:
                    ...
                    for period, file_path in metrics_files.items():
                        if file_path.exists():
                            try:
                                ...
                                print(
                                    f"✅ Loaded {period} metrics for {corrector} run {run_id}"
                                )
    ...
    def aggregate_metrics_across_runs(
        self, all_metrics: Dict[str, Dict[str, pl.DataFrame]]
    ...
    def _compute_weekly_metrics(self, daily_metrics: pl.DataFrame) -> pl.DataFrame:
        ...
        df = daily_metrics.with_columns([pl.col("day").cast(pl.Date)])

        # Add week column
        df = df.with_columns([pl.col("day").dt.strftime("%Y-%W").alias("week")])

        # Get metric columns (excluding day, week, corrector, run_id)
        metric_cols = [
            col
            for col in df.columns
            if col not in ["day", "week", "corrector", "run_id"]
        ]
    ...
    def create_comparison_plots(
        self,
        aggregated_metrics: Dict[str, pl.DataFrame],
        ...
        for period in ["daily", "weekly", "monthly"]:
            ...
            if period == "weekly":
                ...
                daily_metrics = {
                    k: v for k, v in aggregated_metrics.items() if "daily" in k
                }
                ...
                print(
                    f"📊 Computed weekly metrics for {len(period_metrics)} correctors"
                )
            else:
                period_metrics = {
                    k: v for k, v in aggregated_metrics.items() if period in k
                }
    ...
    def create_random_regressor_runs_plots(
        self, all_metrics: Dict[str, Dict[str, pl.DataFrame]], output_dir: str
    ):
        ...
        for period in ["daily", "weekly", "monthly"]:
            ...
            if period == "weekly":
                ...
                if not period_runs:
                    print(
                        "⚠️ No daily data found to compute weekly metrics for random_regressor runs"
                    )
                    continue

                print(
                    f"📊 Computed weekly metrics for {len(period_runs)} random_regressor runs"
                )
            else:
                period_runs = {}
    ...
    def _create_random_regressor_runs_plot(
        ...
        sorted_runs = sorted(
            period_runs.items(), key=lambda x: int(x[0]) if x[0].isdigit() else x[0]
        )
        print(
            f"📊 Processing {len(sorted_runs)} runs: {[run_id for run_id, _ in sorted_runs]}"
        )

        # Debug: Check what data each run contains
        for run_id, df in sorted_runs:
            print(
                f"   Run {run_id}: DataFrame shape {df.shape}, columns: {list(df.columns)}"
            )
        ...
        for i, (run_id, df) in enumerate(sorted_runs):
                ...
                for week_str in sorted_df[time_col].to_list():
                    try:
                        if "-" in week_str and len(week_str.split("-")) == 2:
                            ...
                            x_values.append(
                                pd.to_datetime(
                                    f"{year}-W{week.zfill(2)}-1", format="%Y-W%W-%w"
                                )
                            )
                        else:
                        ...
                        print(
                            f"⚠️ Warning: Could not parse week string '{week_str}': {e}"
                        )
            ...
            print(
                f"   Run {run_id}: mean={y_values.mean():.4f}, std={y_values.std():.4f}, min={y_values.min():.4f}, max={y_values.max():.4f}"
            )
            ...
            if i > 0:
                prev_y_values = (
                    sorted_runs[i - 1][1]
                    .select([time_col, metric_col])
                    .sort(time_col)[metric_col]
                    .to_numpy()
                )
                if len(y_values) == len(prev_y_values):
                    ...
                    if diff < 1e-6:
                        print(
                            f"     ⚠️ WARNING: Run {run_id} is essentially identical to previous run!"
                        )

        # Customize the plot
        ax.set_title(
            f"Random Regressor Individual Runs - {metric.upper()} {variable} ({period.capitalize()})",
            fontsize=14,
            fontweight="bold",
        )
    ...
    def _create_single_comparison_plot(
        ...
        # Customize the plot
        ax.set_title(
            f"{metric.upper()} Comparison - {variable} ({period.capitalize()})",
            fontsize=14,
            fontweight="bold",
        )
        ax.set_xlabel("Time", fontsize=12)
                ...
                for variable in self.variables:
                    ...
                    if len(subset) > 0:
                        ...
                        for row in subset.iter_rows(named=True):
                            print(
                                f"{row['Corrector']:15} | {row['Period']:8} | "
