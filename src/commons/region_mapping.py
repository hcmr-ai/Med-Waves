...
REGION_TO_ID = {"atlantic": 0, "mediterranean": 1, "eastern_med": 2}
...
REGION_DISPLAY_NAMES = {0: "Atlantic", 1: "Mediterranean", 2: "Eastern Mediterranean"}
...
class RegionMapper:
    ...
    def get_region_id(region_name: str) -> int:
        ...
        if region_name not in REGION_TO_ID:
            raise ValueError(
                f"Unknown region: {region_name}. Valid regions: {list(REGION_TO_ID.keys())}"
            )
    ...
    def get_region_name(region_id: int) -> str:
        ...
        if region_id not in ID_TO_REGION:
            raise ValueError(
                f"Unknown region ID: {region_id}. Valid IDs: {list(ID_TO_REGION.keys())}"
            )
