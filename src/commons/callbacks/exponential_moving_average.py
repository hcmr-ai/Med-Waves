...
class EMAWeightAveraging(Callback):
    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx):
        ...
        if self.ema_weights is None:
            self.ema_weights = [
                p.detach().clone() for p in pl_module.parameters() if p.requires_grad
            ]
        else:
            for ema_w, model_w in zip(self.ema_weights, pl_module.parameters(), strict=False):
    ...
    def on_validation_start(self, trainer, pl_module):
        if self.ema_weights is not None:
            ...
            for model_w, ema_w in zip(pl_module.parameters(), self.ema_weights, strict=False):
    ...
    def on_validation_end(self, trainer, pl_module):
        ...
        if hasattr(self, "model_weights"):
            ...
            for model_w, old_w in zip(pl_module.parameters(), self.model_weights, strict=False):
    ...
    def on_load_checkpoint(self, trainer, pl_module, checkpoint):
        if "ema_weights" in checkpoint and checkpoint["ema_weights"] is not None:
            self.ema_weights = [
                w.to(pl_module.device) for w in checkpoint["ema_weights"]
            ]
