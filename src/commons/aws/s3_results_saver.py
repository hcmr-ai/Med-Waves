...
class S3ResultsSaver:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.prefix = (
            self.s3_config.get("prefix", "experiments")
            + "/"
            + self.output_config.get("experiment_name", "")
        )
        ...
        if self.enabled and BOTO3_AVAILABLE:
            ...
            logger.error(
                "S3 saving enabled but boto3 not available. Install boto3 to use S3 functionality."
            )
            self.enabled = False
    ...
    def upload_file(self, local_path: str, s3_key: str = None) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Uploading {local_path.name} ({file_size / (1024**2):.1f} MB) to s3://{self.bucket}/{s3_key}"
            )
    ...
    def _upload_large_file(self, local_path: str, s3_key: str):
        ...
        try:
            ...
            self.s3_client.upload_file(local_path, self.bucket, s3_key, Config=config)
        except Exception as e:
            logger.error(f"Multipart upload failed: {e}")
            raise

    def upload_directory(
        self, local_dir: str, s3_prefix: str = None
    ) -> Dict[str, bool]:
        """
        ...
        logger.info(
            f"Directory upload completed: {successful_uploads}/{total_files} files uploaded successfully"
        )
        return results
