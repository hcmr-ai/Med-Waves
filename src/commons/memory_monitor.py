...
class MemoryMonitor:
    ...
    def get_process_memory_info(self) -> Dict[str, float]:
        ...
        try:
            ...
            return {
                "rss_gb": memory_info.rss
                / (1024**3),  # Resident Set Size (physical memory)
                "vms_gb": memory_info.vms
                / (1024**3),  # Virtual Memory Size (allocated memory)
    ...
    def log_memory_usage(self, stage: str, log_level: int = logging.INFO):
        ...
        if self.log_detailed:
            ...
            logger.log(
                log_level, f"  - RSS (Physical): {process_info['rss_gb']:.2f} GB"
            )
            logger.log(log_level, f"  - VMS (Virtual): {process_info['vms_gb']:.2f} GB")
            logger.log(
                log_level, f"  - Memory %: {process_info['memory_percent']:.1f}%"
            )
            logger.log(log_level, f"  - Threads: {process_info['num_threads']}")
        else:
            logger.log(
                log_level,
                f"Memory usage at {stage}: {process_info['rss_gb']:.2f} GB (RSS)",
            )
    ...
    def log_system_memory(self, stage: str = "system", log_level: int = logging.INFO):
        ...
        logger.log(
            log_level,
            f"  - Used: {system_info['used_gb']:.2f} GB ({system_info['percent_used']:.1f}%)",
        )
    ...
    def _log_to_comet(
        self, stage: str, process_info: Dict[str, float], system_info: Dict[str, float]
    ):
    ...
    def log_memory_summary(self, stage: str = "summary"):
        ...
        if summary["process"]:
            ...
            logger.info(
                f"Process Memory: {proc['rss_gb']:.2f} GB RSS, {proc['vms_gb']:.2f} GB VMS"
            )

        if summary["system"]:
            sys = summary["system"]
            logger.info(
                f"System Memory: {sys['used_gb']:.2f} GB used of {sys['total_gb']:.2f} GB total"
            )
