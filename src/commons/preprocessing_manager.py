...
class PreprocessingManager:
    ...
    def _initialize_preprocessing(self):
        ...
        if scaler_type is None or scaler_type == "null":
            if use_regional_scaling:
                self.logger.warning("Cannot use regional scaling with null scaler. Disabling regional scaling.")
    ...
    def _validate_config(self):
        ...
        if (scaler_type is None or scaler_type == "null") and use_regional_scaling:
            self.logger.warning("Configuration conflict: scaler is 'null' but regional_scaling is enabled.")
            self.logger.warning("Regional scaling requires a base scaler. Consider:")
            self.logger.warning("  - Set scaler to 'standard', 'robust', or 'minmax' for regional scaling")
            self.logger.warning("  - Set regional_scaling.enabled to false to disable regional scaling")
    
    def fit_preprocessing(self, X_train: np.ndarray, regions_train: Optional[np.ndarray] = None) -> None:
        """
    ...
    def transform_data(self, X: np.ndarray, regions: Optional[np.ndarray] = None) -> np.ndarray:
        """
    ...
    def preprocess_splits(self, X_train: np.ndarray, X_val: np.ndarray, X_test: np.ndarray,
                         regions_train: Optional[np.ndarray] = None,
                         regions_val: Optional[np.ndarray] = None,
                         regions_test: Optional[np.ndarray] = None) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
    ...
    def preprocess_splits_memory_efficient(self, X_train: np.ndarray, X_val: np.ndarray, X_test: np.ndarray,
                                          regions_train: Optional[np.ndarray] = None,
                                          regions_val: Optional[np.ndarray] = None,
                                          regions_test: Optional[np.ndarray] = None) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
    ...
    def get_preprocessing_info(self) -> Dict[str, Any]:
        ...
        info = {
            'scaler_type': self.feature_config.get("scaler", "standard"),
            'regional_scaling_enabled': self.feature_config.get("regional_scaling", {}).get("enabled", False),
    ...
    def log_preprocessing_info(self):
        ...
        self.logger.info(f"  Regional scaling: {'Enabled' if info['regional_scaling_enabled'] else 'Disabled'}")
        
        if info['has_scaler']:
            self.logger.info(f"  Scaler: {info['scaler_class']}")
        
        if info['has_regional_scaler']:
            self.logger.info(f"  Regional scaler: {info['regional_scaler_class']} (base: {info['regional_scaler_base']})")
    ...
    def inverse_transform(self, X: np.ndarray, regions: Optional[np.ndarray] = None) -> np.ndarray:
        """
