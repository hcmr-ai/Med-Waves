...
class SamplingManager:
    ...
    def apply_sampling(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        if max_samples is None or sampling_strategy == "none":
            self.logger.debug(
                f"No sampling applied - using all {original_size:,} samples"
            )
            return df
        ...
        if sampling_strategy == "per_location":
            sampled_df = self._apply_per_location_sampling(
                df, max_samples, sampling_seed
            )
            ...
            sampled_df = self._apply_robust_stratified_sampling(
                df, max_samples, sampling_seed
            )
        elif sampling_strategy == "single_region_robust":
            sampled_df = self._apply_single_region_robust_sampling(
                df, max_samples, sampling_seed
            )
        elif sampling_strategy == "per_point_stratified":
            sampled_df = self._apply_per_point_stratification(
                df, max_samples, sampling_seed
            )
        else:
    ...
    def _apply_per_location_sampling(
        self, df: pl.DataFrame, max_samples: int, sampling_seed: int
    ) -> pl.DataFrame:
    ...
    def _apply_robust_stratified_sampling(
        self, df: pl.DataFrame, max_samples: int, sampling_seed: int
    ) -> pl.DataFrame:
        """
        ...
        """
        self.logger.info(
            "  - Robust stratified sampling: basin quotas with percentile-based wave height bins"
        )
        ...
        self.logger.info(
            f"  - Time information available: {len(unique_hours)} unique hours"
        )
        ...
        for basin_name, basin_quota in basin_quotas.items():
            ...
            if len(basin_df) > 0:
                self.logger.info(
                    f"  - Processing {basin_name}: {len(basin_df):,} samples → target {basin_quota:,}"
                )
                ...
                if len(basin_sampled) > 0:
                    ...
                    self.logger.info(
                        f"    {basin_name}: {len(basin_df):,} → {len(basin_sampled):,} samples"
                    )
        ...
        if sampled_dfs:
            ...
            self.logger.warning(
                "No samples found for any basin, returning original data"
            )
        ...
        self.logger.info(
            f"  - Robust stratified sampling: {len(df)} → {len(sampled_df)} samples"
        )

        return sampled_df

    def _sample_basin_with_percentiles(
        self,
        ...
        self.logger.debug(
            f"    {basin_name} targets: low={low_target}, mid={mid_target}, high={high_target}"
        )
    ...
    def _sample_high_bin_with_rough_bias(
        self,
        ...
        if len(very_rough_df) > 0:
            very_rough_sampled = self._sample_with_hourly_coverage(
                ...
                very_rough_target,
                sampling_seed,
                f"{basin_name}_very_rough",
            )
            sampled_dfs.append(very_rough_sampled)

        # Sample remaining high conditions
        other_high_df = high_df.filter(
            (pl.col("vhm0_y") > 2.0) & (pl.col("vhm0_y") < 2.5)
        )
        ...
        if total_sampled < high_target and len(high_df) > total_sampled:
            ...
            if sampled_dfs:
                try:
                    ...
                    remaining_df = high_df.filter(
                        ~pl.col("vhm0_y").is_in(sampled_values)
                    )
                except Exception as e:
                    self.logger.warning(
                        f"    Error filtering sampled values: {e}, using all remaining data"
                    )
    ...
    def _sample_with_hourly_coverage(
        self, df: pl.DataFrame, target: int, sampling_seed: int, stratum_name: str
    ) -> pl.DataFrame:
        """
        ...
        if n_hours == 0:
            ...
            self.logger.warning(
                f"    {stratum_name}: No hour information found, using random sampling"
            )
        ...
        for i, hour in enumerate(unique_hours):
            ...
            if len(hour_df) > 0:
                ...
                hour_sampled = hour_df.sample(
                    n=actual_target, seed=sampling_seed + hour
                )
    ...
    def _calculate_target_samples_per_stratum(
        self, df: pl.DataFrame, max_samples: int
    ) -> Dict[str, int]:
        ...
        for stratum in atlantic_high_waves:
            if stratum_counts[stratum] > 0:
                target_samples_per_stratum[stratum] = min(
                    min_atlantic_high, stratum_counts[stratum]
                )
            ...
            for stratum, count in non_zero_strata.items():
                if stratum not in target_samples_per_stratum:
                    proportional_allocation = int(
                        (count / total_available) * remaining_samples
                    )
                    target_samples_per_stratum[stratum] = min(
                        proportional_allocation, count
                    )
    ...
    def _log_basin_distribution(self, df: pl.DataFrame, stage: str):
        ...
        for basin in ["atlantic", "mediterranean", "eastern_med"]:
            ...
            if len(basin_df) > 0:
                ...
                rough_count = len(
                    basin_df.filter(
                        (pl.col("vhm0_y") >= 2.5) & (pl.col("vhm0_y") <= 4.0)
                    )
                )
                very_rough_count = len(basin_df.filter(pl.col("vhm0_y") > 4.0))

                self.logger.info(
                    f"    {basin}: {len(basin_df):,} samples ({basin_percent:.1f}%) - "
    ...
    def get_sampling_info(self) -> Dict[str, Any]:
        ...
        if sampling_strategy == "per_location":
            sampling_info["samples_per_location"] = self.feature_config.get(
                "samples_per_location", 20
            )
        elif sampling_strategy == "temporal":
            sampling_info["samples_per_hour"] = self.feature_config.get(
    ...
    def log_sampling_config(self):
        ...
        if sampling_info["enabled"]:
            self.logger.info("Sampling configuration:")
            self.logger.info(f"  - Strategy: {sampling_info['strategy']}")
            self.logger.info(
                f"  - Max samples per file: {sampling_info['max_samples_per_file']:,}"
            )
            self.logger.info(f"  - Seed: {sampling_info['seed']}")

            if sampling_info["strategy"] == "per_location":
                self.logger.info(
                    f"  - Samples per location: {sampling_info['samples_per_location']}"
                )
            elif sampling_info["strategy"] == "temporal":
                self.logger.info(
                    f"  - Samples per hour: {sampling_info['samples_per_hour']}"
                )
            elif sampling_info["strategy"] == "robust_stratified":
                self.logger.info(f"  - Basin quotas: {sampling_info['basin_quotas']}")
                self.logger.info(
                    f"  - Wave height distribution: {sampling_info['wave_height_distribution']}"
                )
                self.logger.info(
                    f"  - High wave bias: {sampling_info['high_wave_bias']}"
                )
                self.logger.info(
                    f"  - Hourly coverage: {sampling_info['hourly_coverage']}"
                )
        else:
            self.logger.info("No sampling configured - using all available data")

    def estimate_sampling_reduction(
        self, total_files: int, total_samples: int
    ) -> Dict[str, Any]:
        """
        ...
        reduction_percent = (
            (estimated_original - total_samples) / estimated_original
        ) * 100
    ...
    def _apply_single_region_robust_sampling(
        self, df: pl.DataFrame, max_samples: int, sampling_seed: int
    ) -> pl.DataFrame:
        """
        ...
        self.logger.info(
            f"  - Single-region robust sampling: {len(df):,} samples → target {max_samples:,}"
        )
        ...
        self.logger.info(
            f"  - Wave height bins: Low={len(low_df):,}, Mid={len(mid_df):,}, High={len(high_df):,}"
        )
        ...
        self.logger.info(
            f"  - Target samples: Low={low_target:,}, Mid={mid_target:,}, High={high_target:,}"
        )
        ...
        if len(low_df) > 0:
            low_sampled = self._sample_with_hourly_coverage(
                low_df, low_target, sampling_seed, "Low"
            )
            if len(low_sampled) > 0:
                sampled_dfs.append(low_sampled)
                self.logger.info(
                    f"    Low bin: {len(low_df):,} → {len(low_sampled):,} samples"
                )

        # Sample mid bin
        if len(mid_df) > 0:
            mid_sampled = self._sample_with_hourly_coverage(
                mid_df, mid_target, sampling_seed, "Mid"
            )
            if len(mid_sampled) > 0:
                sampled_dfs.append(mid_sampled)
                self.logger.info(
                    f"    Mid bin: {len(mid_df):,} → {len(mid_sampled):,} samples"
                )

        # Sample high bin with rough/very-rough bias
        if len(high_df) > 0:
            high_sampled = self._sample_high_bin_with_rough_bias(
                high_df, high_target, sampling_seed, sampled_dfs
            )
            if len(high_sampled) > 0:
                sampled_dfs.append(high_sampled)
                self.logger.info(
                    f"    High bin: {len(high_df):,} → {len(high_sampled):,} samples"
                )
        ...
        if sampled_dfs:
            ...
            self.logger.warning(
                "No samples found for any wave height bin, returning original data"
            )
        ...
        self.logger.info(
            f"  - Single-region robust sampling: {len(df)} → {len(sampled_df)} samples"
        )
    ...
    def _log_wave_height_distribution(self, df: pl.DataFrame, prefix: str) -> None:
        ...
        very_rough_count = len(
            df.filter((pl.col("vhm0_y") > q66) & (pl.col("vhm0_y") >= 4.0))
        )
        ...
        self.logger.info(
            f"    Low (≤{q33:.3f}): {low_count:,} ({low_count / total * 100:.1f}%)"
        )
        self.logger.info(
            f"    Mid ({q33:.3f}-{q66:.3f}): {mid_count:,} ({mid_count / total * 100:.1f}%)"
        )
        self.logger.info(
            f"    High (>{q66:.3f}): {high_count:,} ({high_count / total * 100:.1f}%)"
        )
        self.logger.info(
            f"      Rough (2.5-4.0m): {rough_count:,} ({rough_count / total * 100:.1f}%)"
        )
        self.logger.info(
            f"      Very-rough (>4.0m): {very_rough_count:,} ({very_rough_count / total * 100:.1f}%)"
        )

    def _apply_per_point_stratification(
        self, df: pl.DataFrame, max_samples: int, sampling_seed: int
    ) -> pl.DataFrame:
        """
        ...
        """
        from src.data_engineering.per_point_stratification import (
            apply_per_point_stratification,
        )

        self.logger.info(
            "  - Per-point wave height stratification: balanced samples per location across wave height bins"
        )
        ...
        self.logger.info(
            f"SamplingManager passing max_samples: {max_samples:,} to per-point stratification"
        )
