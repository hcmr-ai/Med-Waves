...
class DataLoader:
    ...
    def load_data(
        self, data_paths: Union[str, List[str]]
        ...
        if parallel_loading and len(all_files) > 1:
            ...
            with ProcessPoolExecutor(max_workers=max_workers) as executor:
                ...
                future_to_file = {
                    executor.submit(
                        _load_single_file_worker_for_dataloader,
                        (file_path, self.feature_config, self.sampling_manager),
                    ): file_path
            ...
            for file_path in tqdm(all_files, desc="Loading files"):
                file_path, df, success = _load_single_file_worker_for_dataloader(
                    (file_path, self.feature_config, self.sampling_manager)
                )
        ...
        self.logger.info(
            f"Successfully loaded {len(successful_files)} out of {len(all_files)} files"
        )
    ...
    def _log_total_distribution_summary(
        self, combined_df: pl.DataFrame, successful_files: List[str]
    ) -> None:
        ...
        self.logger.info("=" * 80)
        self.logger.info(
            f"Total samples across {len(successful_files)} files: {total_samples:,}"
        )
        self.logger.info("")
        ...
        for bin_name, (min_val, max_val) in wave_bins.items():
            if max_val == float("inf"):
                ...
                bin_df = combined_df.filter(
                    (pl.col(wave_height_col) >= min_val)
                    & (pl.col(wave_height_col) < max_val)
                )
        ...
        if extreme_count > 0:
            extreme_df = combined_df.filter(
                pl.col(wave_height_col) >= wave_bins["extreme"][0]
            )
            max_wave = float(extreme_df.select(pl.col(wave_height_col).max()).item())
            mean_extreme = float(
                extreme_df.select(pl.col(wave_height_col).mean()).item()
            )

            self.logger.info("")
            self.logger.info("  Extreme wave statistics:")
        ...
        if lat_col and lon_col:
            ...
            mediterranean = combined_df.filter(
                (pl.col(lon_col) >= -6.0) & (pl.col(lon_col) <= 25.0)
            )
            ...
            for region_name, region_df in [
                ...
                ("Eastern Med", eastern_med),
            ]:
                count = len(region_df)
                percentage = (count / total_samples) * 100
                self.logger.info(
                    f"    {region_name:15}: {count:8,} samples ({percentage:5.1f}%)"
                )
    ...
    def _get_wave_height_bins_from_config(self) -> Dict[str, Tuple[float, float]]:
        ...
        stratification_bins = self.feature_config.get(
