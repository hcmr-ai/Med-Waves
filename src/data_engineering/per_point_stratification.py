...
class PerPointStratification:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.global_targets = self.feature_config.get(
            "per_point_stratification_bins",
            {
                "calm": {
                    "range": [0.0, 1.0],
                    "global_target_percentage": 0.15,
                },  # 15% (0-1m) - reduced
                "moderate": {
                    "range": [1.0, 3.0],
                    "global_target_percentage": 0.25,
                },  # 25% (1-3m) - reduced
                "rough": {
                    "range": [3.0, 6.0],
                ...
                "high": {
                    "range": [6.0, 9.0],
                    "global_target_percentage": 0.20,
                    "keep_all": True,
                },  # 20% (6-9m) - new bin for high waves
                "extreme": {
                    "range": [9.0, ".inf"],
                    "global_target_percentage": 0.02,
                    "keep_all": True,
                },  # 2% (>9m) - KEEP ALL SAMPLES
            },
        )

        # Extract wave bins from global targets
        self.wave_bins = {
            name: config["range"] for name, config in self.global_targets.items()
        }

        # Maximum total samples per file
        self.max_samples_per_file = self.feature_config.get(
            "max_samples_per_file", 1000000
        )

        # Debug logging
        self.logger.info(
            f"PerPointStratification initialized with max_samples_per_file: {self.max_samples_per_file:,}"
        )
    ...
    def apply_per_point_stratification(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        self.logger.info(
            f"Applying per-point wave height stratification with global quotas: {original_size:,} samples"
        )
        ...
        self.logger.info(
            f"Found {n_locations:,} unique locations across {len(unique_regions)} regions"
        )
        ...
        for bin_name, bin_config in self.global_targets.items():
            ...
            self.logger.info(
                f"  - {bin_name} bin: {available_samples:,} available, target: {global_target:,} ({target_percentage * 100:.0f}%)"
            )
            ...
            if available_samples > 0:
                ...
                keep_all_samples = (
                    bin_name == "extreme"
                    or bin_name == "ultra_extreme"
                    or bin_config.get("keep_all", False)
                )
                ...
                if keep_all_samples:
                    ...
                    self.logger.info(
                        f"    → Keeping ALL {actual_samples:,} {bin_name} samples (rare conditions)"
                    )
                else:
                    actual_samples = min(global_target, available_samples)
                    sampled_bin = bin_df.sample(
                        n=actual_samples,
                        seed=self.sampling_seed + hash(bin_name) % (2**32 - 1),
                    )
        ...
        if sampled_dfs:
            ...
            self.logger.info(
                f"Per-point stratification completed: {original_size:,} → {final_size:,} samples ({reduction_percent:.1f}% reduction)"
            )
            ...
            if "region" in stratified_df.columns:
                ...
                self.logger.info(
                    "Dropped 'region' column after sampling - region was only used for geographic balance"
                )
            ...
            if "region" in df.columns:
                df = df.drop("region")
                self.logger.info(
                    "Dropped 'region' column - region was only used for sampling"
                )
            return df
    ...
    def _add_region_mapping(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        return df.with_columns(
            [
                pl.struct(["lat", "lon"])
                .map_elements(map_region_from_coords, return_dtype=pl.Int32)
                .alias("region")
            ]
        )
    ...
    def _add_wave_height_bins(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        return df.with_columns(
            [
                pl.col("vhm0_y")
                .map_elements(classify_wave_height, return_dtype=pl.Utf8)
                .alias("wave_height_bin")
            ]
        )

    def _ensure_geographic_balance(
        self, df: pl.DataFrame, unique_regions: list
    ) -> pl.DataFrame:
        """
        ...
        self.logger.info(
            f"  - Preserving ALL {extreme_count:,} extreme waves (vhm0_y >= {extreme_min})"
        )
        self.logger.info(
            f"  - Balancing {non_extreme_count:,} non-extreme waves across regions"
        )
        ...
        self.logger.info(
            f"  - Geographic balance: {samples_per_region:,} non-extreme samples per region"
        )
        ...
        for region in unique_regions:
            ...
            if region_samples > 0:
                ...
                sampled_region = region_df.sample(
                    n=actual_samples,
                    seed=self.sampling_seed + hash(region) % (2**32 - 1),
                )
                balanced_dfs.append(sampled_region)
                self.logger.info(
                    f"    → Region {region}: {actual_samples:,} non-extreme samples (from {region_samples:,} available)"
                )
            else:
                self.logger.warning(
                    f"    → Region {region}: No non-extreme samples available"
                )
    ...
    def _log_wave_height_distribution(self, df: pl.DataFrame) -> None:
        ...
        for row in bin_counts.iter_rows(named=True):
            ...
            self.logger.info(
                f"  {bin_name:12} ({range_str:>8}): {count:6,} samples ({percentage:5.1f}%)"
            )
    ...
    def _log_regional_distribution(self, df: pl.DataFrame) -> None:
        ...
        for row in regional_counts.iter_rows(named=True):
            ...
            percentage = (count / total_samples) * 100
            self.logger.info(
                f"  Region {region}: {count:6,} samples ({percentage:5.1f}%)"
            )
    ...
    def log_stratification_config(self) -> None:
        ...
        self.logger.info(
            "Per-point wave height stratification configuration (global quotas):"
        )
        ...
        for bin_name, (min_val, max_val) in self.wave_bins.items():
            target_percentage = self.global_targets[bin_name][
                "global_target_percentage"
            ]
            ...
            if max_val == float("inf"):
                ...
                range_str = f"{min_val}-{max_val}m"
            self.logger.info(
                f"    {bin_name:12} ({range_str:>8}): {target_samples:6,} samples ({target_percentage * 100:4.0f}%)"
            )


def apply_per_point_stratification(
    df: pl.DataFrame, config: Dict[str, Any]
) -> pl.DataFrame:
    """
