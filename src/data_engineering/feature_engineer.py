...
class FeatureEngineer:
    ...
    def _add_cluster_id_feature_extended(
        ...
        if mode == "grid":
            ...
            self.logger.info(
                f"✅ Using POINT mode: 1 cluster per unique point (precision={precision})"
            )
        ...
        self.logger.info("Added cluster_id feature:")
        ...
        self.logger.info(
            f"   Sample distribution: min={min_samples}, median={median_samples:.0f}, max={max_samples}"
        )
        ...
        if n_small > 0:
            ...
            if mode == "point":
                self.logger.warning(
                    "   Consider using grid mode or merging small clusters"
                )
