...
class ModelEvaluator:
    ...
    def plot_rmse_maps(self):
        ...
        try:
            lat_grid, lon_grid = load_coordinates_from_parquet(
                "s3://" + self.test_files[0], subsample_step=self.subsample_step
            )
        ...
        if rmse_baseline is not None:
            ...
            # Create binary colormap with explicit boundary at 0
            colors = ["#0000FF", "#FF0000"]  # Blue for negative, Red for non-negative
            cmap_binary = LinearSegmentedColormap.from_list(
                "improvement", colors, N=256
            )
            cmap_binary.set_bad("white")

            # Create a norm that ensures boundary is exactly at zero
            # This ensures: <0 → blue, >=0 → red
            ...
            plot_spatial_rmse_map(
                ...
                lon_grid,
                improvement,
                save_path=self.output_dir / "rmse_improvement_binary.png",
                title="RMSE Improvement (Reference - Model)",
                vmin=-imp_abs_max,
                vmax=imp_abs_max,
                cmap=cmap_binary,
            ...
            plot_spatial_rmse_map(
                ...
                lon_grid,
                improvement_mae,
                save_path=self.output_dir / "mae_improvement_binary.png",
    ...
    def plot_model_better_percentage(self, sea_bin_metrics: Dict[str, Dict]):
        ...
        for bin_config in sorted_bins:
            bin_name = bin_config["name"]
            if (
                bin_name in sea_bin_metrics
                and sea_bin_metrics[bin_name].get("count", 0) > 0
            ):
        ...
        plt.tight_layout()
        plt.savefig(
            self.output_dir / "model_better_percentage.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
    ...
    def plot_sea_bin_metrics(self, sea_bin_metrics: Dict[str, Dict]):
        ...
        fig.suptitle(
            ...
            if self.target_column == "corrected_VTM02"
        ...
        for i, (v1, v2) in enumerate(
            zip(baseline_rmse_values, rmse_values, strict=False)
        ):
            if v1 > 0:
                axes[0, 0].text(
                    i - width / 2,
        ...
        # Add value labels on bars
        for i, (v1, v2) in enumerate(
            zip(baseline_mae_values, mae_values, strict=False)
        ):
            if v1 > 0:
                axes[0, 1].text(
                    i - width / 2,
        ...
        print(
            f"Saved sea-bin performance plot to {self.output_dir / 'sea_bin_performance.png'}"
        )
    ...
    def plot_error_distribution_histograms(self):
        ...
        for idx, bin_config in enumerate(bins_to_plot):
            ...
            model_errors = np.array(
                self.sea_bin_error_samples[bin_name]["model_errors"]
            )
            baseline_errors = np.array(
                self.sea_bin_error_samples[bin_name]["baseline_errors"]
            )
        ...
        plt.tight_layout()
        plt.savefig(
            self.output_dir / "error_distribution_histograms.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
        print(
            f"Saved error distribution histograms to {self.output_dir / 'error_distribution_histograms.png'}"
        )
    ...
    def plot_error_boxplots(self):
        ...
        plt.tight_layout()
        plt.savefig(
            self.output_dir / "error_distribution_boxplots.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
        print(
            f"Saved error distribution box plots to {self.output_dir / 'error_distribution_boxplots.png'}"
        )
    ...
    def plot_error_violins(self):
        ...
        plt.tight_layout()
        plt.savefig(
            self.output_dir / "error_distribution_violins.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
        print(
            f"Saved error distribution violin plots to {self.output_dir / 'error_distribution_violins.png'}"
        )
    ...
    def plot_error_cdfs(self):
        ...
        plt.tight_layout()
        plt.savefig(
            self.output_dir / "error_distribution_cdfs.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
        print(
            f"Saved error CDF plots to {self.output_dir / 'error_distribution_cdfs.png'}"
        )
    ...
    def plot_vhm0_distributions(self):
        ...
        plt.tight_layout()
        plt.savefig(
            self.output_dir / f"{self.var_name}_distributions.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
        print(
            f"Saved VHM0 distribution plot to {self.output_dir / f'{self.var_name}_distributions.png'}"
        )
        ...
        ax.set_ylabel("Density", fontsize=12, fontweight="bold")
        ax.set_title(
            f"{self.var_name_full} Distribution Comparison (Model vs Reference)",
            fontsize=14,
            fontweight="bold",
        )
        ax.legend(fontsize=11, framealpha=0.9, loc="upper right")
        ...
        plt.tight_layout()
        plt.savefig(
            self.output_dir / f"{self.var_name}_distributions_model_vs_reference.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
        print(
            f"Saved VHM0 distribution plot to {self.output_dir / '{self.var_name}_distributions_model_vs_reference.png'}"
        )
        ...
        ax.set_ylabel("Density", fontsize=12, fontweight="bold")
        ax.set_title(
            f"{self.var_name_full} Distribution Comparison (Reference vs Uncorrected)",
            fontsize=14,
            fontweight="bold",
        )
        ax.legend(fontsize=11, framealpha=0.9, loc="upper right")
        ...
        plt.savefig(
            self.output_dir
            / f"{self.var_name}_distributions_reference_vs_uncorrected.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
        print(
            f"Saved VHM0 distribution plot to {self.output_dir / f'{self.var_name}_distributions_reference_vs_uncorrected.png'}"
        )
