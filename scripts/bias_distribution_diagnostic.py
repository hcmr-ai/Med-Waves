...
def weighted_stats(hist: np.ndarray, centers: np.ndarray) -> dict:
    ...
    return dict(n=int(total), mean=mean, std=std,
...
def main():
    ...
    print("=" * 70)
    hdr = (f"  {'Split':<12} {'n':>14} {'mean':>7} {'std':>7} "
           f"{'p25':>7} {'p50':>7} {'p75':>7} {'p90':>7} {'p95':>7} {'p99':>7}")
    print(hdr)
    print("  " + "-" * 85)
    for key, acc in accumulators.items():
        s = acc.stats()
        print(f"  {key:<12} {s['n']:>14,} {s['mean']:>7.4f} {s['std']:>7.4f} "
    ...
    for test_key, test_acc in ((k, v) for k, v in accumulators.items() if k != "train"):
        ...
        print(f"\n  train vs {test_key}:")
        print(f"    VHM0  KL(train||test)={kl_v:.4f}  overlap={ov_v:.4f}")
        print(f"    bias  KL(train||test)={kl_b:.4f}  overlap={ov_b:.4f}")
        print(f"    Per sea-state bias overlap (model coverage):")
        for wi, (lo, hi, name) in enumerate(WAVE_HEIGHT_BINS):
            ...
            print(f"      [{lo:.0f}–{hi:.0f}m] {name:<10}: "
                  f"train n={tr_n:>12,}  test n={te_n:>12,}  overlap={ov:.4f}")
    ...
    # ------------------------------------------------------------------
    # Plot 4 & 5: Per sea-state bin  (one figure per test split)
    # ------------------------------------------------------------------
    for test_key, test_acc in ((k, v) for k, v in accumulators.items() if k != "train"):
        # 4: bias KDE per bin
        ...
        for ax, (wi, (lo, hi, name)) in zip(axes, enumerate(WAVE_HEIGHT_BINS)):
            tr_h = train_acc.wbin_bias_hist[wi]
            te_h = test_acc.wbin_bias_hist[wi]
            _kde_plot(ax, tr_h, _BIAS_CENTERS, x_bias,
                      _split_color("train"),    f"train (n={int(tr_h.sum()):,})")
            _kde_plot(ax, te_h, _BIAS_CENTERS, x_bias,
                      _split_color(test_key), f"{test_key} (n={int(te_h.sum()):,})")
            ov = bhattacharyya_overlap(tr_h, te_h)
            ax.set_title(f"{name}\n[{lo:.0f}–{hi:.0f}m]\noverlap={ov:.3f}", fontsize=9)
            ax.set_xlabel("|bias| [m]", fontsize=8)
            ax.legend(fontsize=7)
            ax.grid(True, alpha=0.3)
        fig.suptitle(
            f"Bias distribution per sea-state — train vs {test_key}  [{args.region}]\n"
            "overlap→1: test bias drawn from same distribution the model trained on",
        ...
        fig, axes = plt.subplots(1, n_wb, figsize=(4 * n_wb, 5), sharey=False)
        for ax, (wi, (lo, hi, name)) in zip(axes, enumerate(WAVE_HEIGHT_BINS)):
            tr_h = train_acc.wbin_vhm0_hist[wi]
            te_h = test_acc.wbin_vhm0_hist[wi]
            x_sub = np.linspace(lo, hi, 100)
            _kde_plot(ax, tr_h, _VHM0_CENTERS, x_sub,
                      _split_color("train"),    f"train (n={int(tr_h.sum()):,})")
            _kde_plot(ax, te_h, _VHM0_CENTERS, x_sub,
                      _split_color(test_key), f"{test_key} (n={int(te_h.sum()):,})")
            ov = bhattacharyya_overlap(tr_h, te_h)
            ax.set_title(f"{name}\n[{lo:.0f}–{hi:.0f}m]\noverlap={ov:.3f}", fontsize=9)
            ...
            ax.grid(True, alpha=0.3)
        fig.suptitle(
            f"VHM0 distribution per sea-state — train vs {test_key}  [{args.region}]\n"
            "overlap→1: test VHM0 falls within the range the model was trained on",
            fontsize=11, fontweight="bold")
        fig.tight_layout()
    ...
    print("  Plot 04 / per-bin bias overlap:")
